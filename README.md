...
アプリの「新規登録」から表示名・メール・8文字以上のパスワードを入力します。ローカルの[Mailpit](http://localhost:59324)で確認メールを開き、「メールアドレスを確認する」から登録を完了してください。ローカルメールは外部へ配送されません。seedは空なので、初回起動だけではユーザーや記録は作られません。

メール確認付きの一般登録を提供します。公開環境にはSMTPなどの設定が別途必要です。[登録・公開手順](docs/email-signup.md)を参照してください。管理者による開発アカウントの発行も[管理者登録ガイド](docs/admin-managed-accounts.md)の手順で利用できます。アカウントを作成できたら [最初に試す操作](#最初に試す操作) でログインから記録共有まで確認します。
...
1. メール確認済みのアカウントAでログインし、「グループ」からグループを作成する。
2. 招待コードをコピーし、別ブラウザでメール確認済みのアカウントBでログインしてコードで参加する。
...
活動カレンダーは日ごとの総負荷量（重量×回数の合計）を示します。月内で多い日は赤、少ない日は青、未記録は灰色です。仕様は [活動ヒートマップ](docs/activity-heatmap.md) を参照してください。
...
メール確認付き登録・未確認ログイン拒否・実Mailpitの確認リンク・再送・期限切れと、既存の管理者作成ユーザーの別ブラウザでのログイン・参加・共有、下書き復元、通信失敗後の再送、再ログインを確認します。
...
クラウドの環境変数・DB migration・Auth設定は別途必要です。[メール確認付き登録の公開手順](docs/email-signup.md#公開への反映手順)に従い、SMTPと戻り先を準備してから一般登録を有効にします。確認メールの送信設定はWebのデプロイだけでは反映されません。
