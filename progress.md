...
## 2026-09-25 グループカードの基本形を採用

- 変更内容・目的: ユーザーが調整案2のA「基本」を選択した。分類ラベル `GROUP` は見れば分かるため削除し、グループ名、LIVE、セット数、総負荷だけを残す方針を確定した。
- 影響範囲・関連ファイル: `docs/design/home-screen.md`、`progress.md`。実アプリはまだ変更しない。
- 次のアクション: 採用したカードの高さとトレーニングタイムラインとのつながりを比較する。

## 2026-09-25 ホーム上部の近接レイアウトを採用

- 変更内容・目的: ユーザーがホーム調整案1のA「近接」を選択した。全グループの4列要約の直下へスワイプ式グループカードを置く構成を、以後のホーム検討の固定条件にした。
- 影響範囲・関連ファイル: `docs/design/home-screen.md`、`progress.md`。この段階では実アプリ、API、DBを変更しない。
- 次のアクション: グループカード内のLIVE・セット数・総負荷の配置だけを比較し、採用後に実装へ進む。

## 2026-09-25 現行グループカードを生かすホーム4案を追加

- 変更内容・目的: 黒いLIVE用カードを追加する案を撤回し、現行のグループカードをそのまま残す案、カードを小さくして今日の要約を外へ出す案、LIVEを細い行へ分離する案、カードなしの比較案を作成した。全案でトレーニング全体の種目・重量・回数を時系列に表示する。
- 影響範囲・関連ファイル: `frontend/public/previews/home-current-card-options.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、共有・スタンプ送信は変更しない。
- テスト方針: 独立した静的プレビューのため、採用までアプリテストは追加しない。案切替と狭い幅の表示を確認する。

## 2026-09-25 時系列ホームの詳細表示案を追加

- 変更内容・目的: ユーザーが基本の時系列を選び、トレーニングの種目・重量・回数を省略せず見たいと指定したため、上部の今日の状況、LIVE中の仲間、グループ絞り込み、トレーニング全体の種目明細を1画面へ組み合わせた詳細プレビューを追加した。
- 影響範囲・関連ファイル: `frontend/public/previews/home-activity-timeline-v2.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、共有・スタンプ送信の仕様は変更しない。
- テスト方針: 静的プレビューだけの変更。採用前に、画面高さ、長い種目名・多数種目、LIVEなし、グループなし、取得失敗を追加して確認する。

## 2026-09-25 今日の活動プレビューの案切替を修正

- 変更内容・目的: プレビューの案切替ボタンが参照するHTML属性名とJavaScriptの参照先が一致せず、A〜Dを切り替えられない不具合を修正した。`data-view` と `data-panel` を明示し、ボタン群だけを操作対象に絞った。
- 影響範囲・関連ファイル: `frontend/public/previews/home-activity-options.html`、`progress.md`。実アプリ、API、DB、共有・保存仕様は変更しない。
- テスト方針: ブラウザでA〜Dを順に切り替え、各案が単独で表示されることを確認する。

## 2026-09-25 今日の活動を起点にしたトレーニング単位の4案を追加

- 変更内容・目的: ユーザーの「タイムラインならアイコン不要」「セット単位でなくトレーニング全体を出す」という判断に基づき、今日の活動を起点にした4つの実画面プレビューを追加した。要約＋行、カード、日付ごとの時系列、LIVEをフィード外に分ける構成を比較する。
- 影響範囲・関連ファイル: `frontend/public/previews/home-activity-options.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、共有範囲、スタンプ送信は変更しない。
- テスト方針: 独立した静的プレビューだけの変更のため、アプリテストは追加しない。案・グループ絞り込み・スタンプの見た目をブラウザで比較する。
- 未解決事項・次のアクション: 表示に総負荷を残すか、進行中のトレーニングをフィード外へ分けるか、採用案を確認する。

## 2026-09-25 グループカードなしのホーム・タイムライン4案を追加

- 変更内容・目的: ユーザーの「Instagramのように、仲間のトレーニングを最新順に見てスタンプを押す」「グループで絞り込む」方向に合わせ、グループカードを使わないタイムライン中心の実画面プレビューを追加した。Aはフィルター中心、Bは仲間アイコン列、Cは種目選択と共通の選択行、Dは今日の活動要約を起点とする。
- 影響範囲・関連ファイル: `frontend/public/previews/home-timeline-options.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、スタンプ送信、既存の共有権限は変更しない。
- テスト方針・検証: 独立した静的プレビューのためアプリテストは追加しない。案切替、フィルター選択、スタンプの見た目の切替、暗色切替をブラウザで確認する。A案はヘッドレスChromiumの430px幅で撮影し、フィード、固定START、下部ナビの重なりと横はみ出しがないことを確認した。全グループ横断フィードは採用後の実装範囲として扱う。
- 未解決事項・次のアクション: ユーザーが出発点を選んだ後に、空・LIVEなし・グループなし・取得失敗の状態と、フィルターをグループだけにするか部位まで含めるかを確定する。

## 2026-09-25 ホーム上部の仲間表示を比較する3案を作成

- 変更内容・目的: ユーザー依頼により、トレーニング画面でも使い回せるアイコンと仲間の状態をホーム上部へ置く、A（仲間ドック）・B（アイコン入口）・C（チームボード）の切替式プレビューを作成した。実アプリを変更せず、選んだ案を段階的に調整してから反映するための比較用土台とした。
- 影響範囲・関連ファイル: `frontend/public/previews/home-options.html`、`docs/design/home-screen.md`、`docs/design/README.md`、`docs/README.md`、`progress.md`。API、DB、保存、共有範囲、既存ホーム画面は変更しない。
- テスト方針・検証: 独立した静的プレビューと設計資料だけの変更のため、先行アプリテストは追加しない。3案・テーマ切替の識別子、SVGの開始/終了数、設計資料からのリンク、`git diff --check`を確認した。サンドボックスではローカルHTTP待受を作れないため、ブラウザでの幅別目視は未実施。
- 未解決事項・次のアクション: A/B/Cから出発点を選び、LIVEなし・グループなし・取得失敗時の状態と、再利用するアイコンの範囲を具体化する。採用前に実アプリへは反映しない。

## 2026-09-25
- 変更内容: PR前チェックとして `make check` を実行し、backend lint・103テスト、frontend lint・型検査・99単体テスト・production buildが成功した。終了確認と比較表の対象E2Eを、実機用の3001番サーバーを再利用して実行したが、テストの擬似ログインがLAN向けSupabase Authへ接続するためログイン後の画面へ進めず失敗した。
- 目的: 最新の文言・比較表・スタンプ互換修正をPR前に検証し、実機用サーバーを停止せずE2Eの実行可否を確認する。
- 影響範囲: PR前の検証記録のみ。
- 関連ファイル: `progress.md`、`frontend/tests/e2e/recording-style.spec.ts`。
- 未解決事項: 実機用3001番を停止して専用のE2Eサーバーを起動するまで、ローカルの対象E2Eは未成功。CIは専用環境で全E2Eを実行する。
- 次のアクション: UI・テスト・設計資料をコミットし、既存PRへpushしてCIを確認する。

## 2026-09-25
- 変更内容: トレーニング中の共通ヘッダーと種目選択の終了入口を `トレーニング終了` に短縮した。確認シートでは白い補助操作を `トレーニングに戻る`、赤い確定操作を `終了する` に統一した。
- 目的: 確認操作の文言を短くしつつ、戻る先と確定操作を明確にする。
- 影響範囲: 種目選択・記録入力のヘッダー、終了確認シート、採用プレビュー、関連E2E。
- 関連ファイル: `frontend/src/features/session/session-screen.tsx`、`frontend/src/app/v2.css`、`frontend/tests/e2e/recording-style.spec.ts`、`docs/design-system.md`、`docs/design/training-session.md`。
- 未解決事項: なし。
- 次のアクション: 3幅で終了確認のボタン配置を確認する。

## 2026-09-25
- 変更内容: トレーニング終了の確認シートから、通常時の「おつかれさまでした。保存したセットは履歴で確認できます。」を削除した。通常時はシート見出しと `まだ続ける`、`今日のトレーニング終了` の2操作だけを表示し、未保存入力がある場合だけデータ喪失を防ぐ警告を表示する。
- 目的: 終了確認を余計な説明のない短い操作へ戻す。
- 影響範囲: 記録中の終了確認シート。
- 関連ファイル: `frontend/src/features/session/session-screen.tsx`、`docs/design-system.md`、`docs/design/training-session.md`。
- 未解決事項: なし。
- 次のアクション: 他の画面修正と合わせて対象E2Eを実行する。

## 2026-09-25
- 変更内容: 記録中の `＋ 種目を追加` が開く追加シートで、シート見出しとフォーム内見出しの二重表示を解消した。シートの `種目を追加` を唯一の見出しとし、設定の種目管理画面ではフォーム内見出しを維持する。
- 目的: 同じ画面名の繰り返しをなくし、追加フォームの最初の入力をすぐ見つけられるようにする。
- 影響範囲: 記録中の種目追加シートと、その画面構成を確認するE2E。
- 関連ファイル: `frontend/src/features/exercises/exercise-catalog.tsx`、`frontend/src/features/session/session-screen.tsx`、`frontend/tests/e2e/training-selection.spec.ts`、`docs/design/training-session.md`。
- 未解決事項: 実機向け3001番の開発サーバーと通常のE2Eサーバーは `.next/dev` を共有するため、実機サーバー稼働中は対象E2Eを別途実行する。
- 次のアクション: lint・型検査と、実機画面で見出しが一つだけであることを確認する。

## 2026-09-25
- 変更内容: 種目メモと今日のメモを同じ1行・24pxの直接入力に固定し、押下して編集状態になっても高さ・余白・枠が変わらないようにした。比較表はDOM・グリッドとも「今回 / ゴミ箱 / 前回」の固定列順に統一し、前回なしでもゴミ箱を今回と前回の間へ固定した。見出し、前回日付、全セットコピーも同じ列へ揃えた。既存セットの編集は、一覧の「今回」で選択中の記録値だけを部位選択と同じ薄い赤背景・赤枠で示す。前の表示面積まで疑似要素で広げるため、数値位置は動かない。前回列・ゴミ箱・下部の重量/回数入力欄・数値文字色は変えない。選択状態の横断ルールも部位選択へ統一した。スタンプは表示と同時に取得を始め、仲間の記録詳細では選択を即時表示して閉じた後に送信する。詳細表示は採用プレビューどおり、種類ごとの絵文字と重なった送信者アイコンへ変更した。本人の記録も上部の今日の仲間へ表示し、本人宛てスタンプも送信・遅延受信できるようにした。記録詳細は日付だけを見出しにし、本文の日付と「トレーニング中」を外した。進行中の赤いLIVEドット・最高記録の炎を記録者アイコンへ表示し、スタンプを総負荷・セット数の直下へ移動した。
- 目的: メモ編集時の画面のがたつき、前回有無によるゴミ箱の移動、編集中の対象の分かりにくさ、スタンプを押せるまでの待ちを解消する。
- 影響範囲: 記録画面のメモ、セット比較・編集入力、仲間/本人の記録詳細のスタンプ、関連E2E。
- 関連ファイル: `backend/app/infrastructure/stamps.py`、`backend/tests/test_stamps.py`、`frontend/src/features/session/session-screen.tsx`、`frontend/src/features/v2/shared-workout-detail.tsx`、`frontend/src/features/training/record-list.tsx`、`frontend/src/features/stamps/stamp-control.tsx`、`frontend/src/app/v2.css`、`frontend/tests/unit/record-list.test.tsx`、`frontend/tests/e2e/recording-style.spec.ts`、`frontend/tests/e2e/session-flow.spec.ts`、`frontend/tests/e2e/shared-workout-detail.spec.ts`、`docs/design-system.md`、`docs/design/training-session.md`、`docs/stamps.md`。
- 検証結果: `TEST_DATABASE_URL=.../gotore_test make check`が成功（backend 274件、frontend unit 99件、lint・型検査・production build）。本人の記録へのスタンプ送信を含む`backend/tests/test_stamps.py` 9件、コピー位置・詳細を閉じた後のスタンプ送信を含む対象E2E 6件、全E2E 212件も成功した。全チェック中に検出したRLSテストの検証ロール切替権限不足は、テスト内で作成者へ明示的に付与して解消した。
- 未解決事項: LAN用3001番の実機で、前回なし行の中央ゴミ箱、選択枠、本人アイコンからの本人スタンプと遅延受信を確認する。
- 次のアクション: 開発サーバーを3001番で復帰し、スマホ実機で上記の表示と操作を確認する。PR作成前に変更差分を再確認する。

## 2026-09-25
- 変更内容: 操作を通信待ちで止めない横断ルールを設計資料へ追加した。スタンプ詳細では選択をローカルで即時反映し、閉じる時点の最終状態だけをバックグラウンド送信する実装へ変更中。トレーニング画面は、次の種目・種目名からの選択画面遷移、下部ナビ等で非表示になる時点に未確定入力を端末下書きへ同期する。入力ドックは重量・回数の見出しを各入力欄の真上へ寄せ、推定1RMを二つの上矢印の中央へ置いた。縮小時は見出しも隠す。重複する一覧下の`＋`と編集見出しの`を編集`を外し、新規保存操作を`セットを追加`へ統一した。編集時の色は入力欄だけに限定した。種目候補は左寄せのまま、`何日前`の有無にかかわらず種目名を行の縦中央へ固定した。
- 目的: 通信の遅さでスタンプ、画面遷移、次の種目への操作を止めず、未送信・未確定の操作を失わないようにする。
- 影響範囲: スタンプ送信状態、仲間の記録詳細、トレーニング入力の種目切替・画面遷移、横断デザインルール。
- 関連ファイル: `frontend/src/features/stamps/stamp-store.ts`、`frontend/src/features/stamps/stamp-control.tsx`、`frontend/src/features/session/session-screen.tsx`、`frontend/src/features/session/number-wheel.tsx`、`frontend/src/app/v2.css`、`frontend/tests/unit/stamp-store.test.ts`、`frontend/tests/e2e/input-controls.spec.ts`、`frontend/tests/e2e/training-selection.spec.ts`、`docs/stamps.md`、`docs/design-system.md`、`docs/design/training-session.md`。
- 検証結果: `bun run lint`、`git diff --check`、スタンプ・セッションの単体テスト20件、`input-controls.spec.ts`と`training-selection.spec.ts`のブラウザテスト4件が成功。重量/回数ラベルと入力欄、推定1RMと上矢印の中央線、種目名の行内縦中央を座標で回帰確認した。停止して応答しないLAN向け3001番のNext.jsを終了して再起動し、`http://127.0.0.1:3001`のHTTP 200を確認した。一行メモ化後も`bun run lint`と`git diff --check`、3001番のHTTP 200を確認した。該当E2Eは、稼働中のLAN用Next.jsが`.next/dev`ロックを使うため同時起動できず未実施（実機用サーバーは停止していない）。
- 未解決事項: スタンプ詳細を閉じた後の送信失敗復旧と、種目切替中の下書き復元を実機で確認する。既存E2Eの一部は通常同期表示を期待する古い仕様のため別途整理が必要。
- 次のアクション: 実機でスタンプ選択・詳細閉鎖・次種目切替と、最新の入力ドック表示を確認する。

## 2026-09-25
- 変更内容: スマホでメモとグループメンバーを取得できない事象を確認し、停止していたバックエンドを `make backend` で `0.0.0.0:8000` に復旧した。
- 目的: Next.jsの`/api`経由で利用するメモ・グループAPIの接続を回復する。
- 影響範囲: メモ、グループメンバーを含むバックエンドAPIを使う画面全般。Supabase Auth（59321）とフロントエンド（3001）は停止していなかった。
- 関連ファイル: 実行プロセスのみ（リポジトリのソース変更なし）。
- 検証結果: `0.0.0.0:8000`、`0.0.0.0:3001`の待受を確認。フロントエンドはHTTP 200、バックエンドはルート未定義のため`/health`が404となることを確認し、接続拒否ではない。
- 未解決事項: スマホ実機で再読込後のメモ・グループメンバー表示を確認する。
- 次のアクション: 実機結果を受けて、必要なら認証・API応答を個別に調査する。

## 2026-09-25
- 変更内容: 記録入力の±5矢印だけを24pxへ広げ、数値欄の上下から同じ7pxの間隔になるグリッドへ固定した。`重量 kg`と`回数 回`は入力欄と関連付けず、押しても数字入力へ移らない表示専用の見出しにした。種目メモ・今日のメモは保存/閉じるボタンを廃止し、Enterまたはフォーカス離脱で保存する直接入力へ統一した。部位選択は薄い赤の面と輪郭で押せることを示し、不要な下線を除いた。記録中の`＋ 種目を追加`は設定用一覧ではなく追加フォームを直接開き、追加完了後に種目選択へ戻るようにした。
- 目的: 数字欄の誤タップ、編集時のメモ表示の変化、部位フィルターの分かりにくさ、種目追加時に管理一覧へ逸れる導線を解消する。
- 影響範囲: 記録入力、種目/今日のメモ、種目選択の部位フィルター、記録中の種目追加、採用プレビューと設計資料。
- 関連ファイル: `frontend/src/features/session/inline-memo.tsx`、`frontend/src/features/session/session-screen.tsx`、`frontend/src/features/exercises/exercise-catalog.tsx`、`frontend/src/app/v2.css`、`frontend/tests/e2e/input-controls.spec.ts`、`frontend/tests/e2e/recording-style.spec.ts`、`frontend/tests/e2e/session-flow.spec.ts`、`frontend/tests/e2e/training-selection.spec.ts`、`docs/design/training-session.md`、`frontend/public/previews/training-selection-adopted.html`。
- 検証結果: `bun run lint`、`git diff --check`は成功。`input-controls.spec.ts`の3件（単位見出しを押しても入力へ移らないこと、上下矢印と数値欄の間隔が同じことを含む）と`training-selection.spec.ts`は成功。`recording-style.spec.ts`は今回のメモ操作に到達する前に、既存の「全セットでゴミ箱のx座標が同一」という期待が現行比較行と一致せず3幅で失敗した。`session-flow.spec.ts`も既存の通常時`.sync-status`表示を期待して失敗した（現行仕様は通常時の同期表示なし）。
- 未解決事項: 実機で矢印の当たり判定、メモのフォーカス移動保存、部位フィルターの見た目を確認する。既存E2Eの期待値は、別変更として現行採用仕様へ整理する必要がある。
- 次のアクション: LAN用の開発サーバーを3001番で再起動し、実機確認を受けて残る入力画面を調整する。

## 2026-09-25
- 変更内容: スタンプをグループ単位ではなく記録単位に統合する方針を確定し、migration・API・クライアント状態・DBテストを更新中。
- 目的: 同じ人の同じ記録は、閲覧できるどの所属グループから開いても同じスタンプとして扱い、関係のないグループには記録自体を出さない。
- 影響範囲: `gotore_workout_stamps` の既存重複統合と一意制約、スタンプ取得・集計・取消、選択画面の仲間カード。
- 関連ファイル: `supabase/migrations/20260925010000_global_workout_stamps.sql`、`backend/app/infrastructure/stamps.py`、`backend/tests/test_stamps.py`、`frontend/src/features/stamps/stamp-store.ts`、`docs/stamps.md`、`docs/design/training-session.md`。
- 未解決事項: migrationを実DBへ適用した検証と、全バックエンドDBテストには専用の `TEST_DATABASE_URL` が必要。
- 次のアクション: lint・DBテスト・対象E2Eを実行し、選択画面のスタンプ詳細を採用プレビューに合わせる。
...
## 2026-09-22 テスト実行の高速化へ着手（#192）
- 変更内容・目的: ユーザーからテスト増加と待ち時間への改善依頼。#191と分離しmain b5dd546からchore/test-speedを作成。対象E2E指定、ビルドを省く反復チェック、CIの2 worker化、同一PRの古いCIキャンセルを追加。
- 影響範囲: Makefile、Playwright/CI設定、テスト・開発手順。アプリの業務処理・DB・本番タイマー変更なし。全件実行は維持し、テスト削除なし。
- 検証: 遅いスタンプ受信3幅の実時間待ちだけをClockへ変更。3件成功・テスト本体約16秒（起動等込み22.3秒）、前回ログの実時間版本体約86秒と比較。存在しない対象指定は非0終了を確認。設定・既存テスト高速化につき新しい先行テストは追加せず、元の検証内容で再実行する。
- 未解決事項・次のアクション: 全207件の2 worker実行を計測中。CI・資料・コマンドの検証後に独立PRを作成する。前回の全210件とは件数・環境が異なるため総時間を厳密な同条件比較とは扱わない。
- 追加改善: 資料だけのPRではアプリのCIを省く変更判定を追加。未知のパス・コード・設定・DB・空差分・判定失敗時は全チェックへ回し、renameは移動元も見る。mainでは常に全件。分岐の境界を小さな単体テストとNUL区切りCLI入力で確認。既存の必須ジョブ名は維持する。

## 2026-09-22 2並列で全E2Eを検証
- 検証結果: main起点の全207件が2 workerで成功、7.4分。前回#191の210件・13.1分とは件数や接続失敗の有無が異なるため参考比較とする。テストは削除していない。資料判定の9件とCLI境界、YAML構造、判定失敗時の全実行条件も確認済み。
...
## 2026-09-24 種目選択の再設計プレビューを開始

- 変更内容・目的: ユーザーから、トレーニング開始後の種目選択で文字が多い、上部のスタンプで表示が揺れる、検索・記録表示・追加操作が分かりにくいという指摘を受けた。実アプリへ変更を入れる前に、検索を使わず、よく行う種目・部位・今日の総負荷比較・通知入口を組み替えた複数案を静的プレビューで比較する。
- 影響範囲・関連ファイル: task.md、frontend/public/previews/training-start-options.html。API、DB、既存の種目選択・スタンプ・保存処理は変更しない。
- 検証: 320/390/430pxとライト/ダークで、文字量、横はみ出し、部位・お気に入り・追加導線を確認する。
- 未解決事項・次のアクション: ユーザーが採用案を選び、通知の表示、総負荷の比較基準、追加画面の遷移を決めてから実装範囲を確定する。

## 2026-09-24 PR #194 のdatabase E2E再失敗を修正

- 変更内容・目的: 終了確認に「記録に戻る」を復元した結果、E2Eがシート標準の閉じる操作も含めた総ボタン数を2件と固定して失敗した。個数ではなく、必要な「記録に戻る」と「終了する」が表示・操作できることを個別に検証する形へ変更した。
- 影響範囲・関連ファイル: frontend/tests/e2e/recording-style.spec.ts、progress.md。アプリの終了確認や保存処理は変更しない。
- 検証: GitHub Actionsの失敗ログで3幅すべて同一の期待値不整合を確認。修正後、CIと同じ環境変数でrecording-style E2Eを実行する。
- 未解決事項・次のアクション: E2E成功後、今回のCI修正だけを別コミットにしてPR #194へpushし、GitHub Actionsのdatabaseジョブ成功まで確認する。

## 2026-09-24 種目選択プレビューの方向を絞り込み

- 変更内容・目的: 初回4案の確認を受け、種目選択は「今日鍛える部位を上で選ぶ」「選択部位のよく行う種目を上から選ぶ」という流れへ絞った。負荷比較は分かりにくい“いつもの何％”を使わず、先週の同曜日までの総負荷との比較へ置き換える。記録入力画面の白地・赤い操作・短い情報量に寄せた次案を作成する。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options.html、progress.md。実アプリ、API、DB、種目候補の並び順は変更しない。
- 検証: 390pxで3案の描画、胸・腕の複数選択状態、横はみ出しなし（scrollWidth 390px）をChromiumで確認した。プレビュー内のブラウザ予約語との変数名衝突で空白になった表示も修正した。320/430pxは採用案を絞った後に確認する。
- 未解決事項・次のアクション: ユーザーが部位フィルターと現在セット要約の採用形を選んでから、実データの集計範囲と種目の並び替え規則を定義する。

## 2026-09-24 種目選択プレビューの情報量を削減

- 変更内容・目的: ユーザー確認により、「よく行う種目」「選ぶ」「複数選べます」などの説明を画面から外す。部位は横スライドでなく全8個を常時表示し、選択状態は色だけで示す。最上部に今日のメニューを置き、数値は総負荷・セット数・先週差へ絞った案を追加する。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v3.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: 390pxで、全8部位、複数選択、今日のメニュー、総負荷・セット数を一画面内で確認する。
- 未解決事項・次のアクション: ユーザーの「総負荷セットじゃないと」の続きの意図を、プレビュー確認時に必要なら補足してもらう。採用後に実データ集計と表示規則を決める。

## 2026-09-24 セット削除の位置を今回列へ固定

- 変更内容・目的: ユーザー指摘により、前回セットの文字量でゴミ箱の位置が動いて見える問題を修正する。削除操作を今回セットのセル右端へ絶対配置し、前回列の内容量と無関係に各行の同じ位置へ固定した。
- 影響範囲・関連ファイル: frontend/src/app/v2.css、frontend/tests/e2e/recording-style.spec.ts、progress.md。セットの編集・削除・前回コピー・保存処理は変更しない。
- 検証: 前回値があるセット1と、前回値がないセット4のゴミ箱のx座標が同じであるE2Eを追加し、既存の記録入力E2Eとfrontendチェックを実行する。
- 未解決事項・次のアクション: 成功後、プレビュー用の未確定ファイルを含めず、この修正だけをPR #194へ追加commit/pushする。

## 2026-09-24 終了確認の文言と今日のメニュー表示を調整

- 変更内容・目的: ユーザー指定により、終了確認の取消操作を「まだ続ける」、確定操作を「今日のトレーニング終了」へ変更した。種目選択のプレビューでは、今日のメニューに全セットを横一列で置く案へ変更した。
- 影響範囲・関連ファイル: frontend/src/features/session/session-screen.tsx、frontend/src/app/v2.css、frontend/tests/e2e/recording-style.spec.ts、frontend/public/previews/training-selection-options-v3.html、progress.md。
- 検証: 終了確認の新しい2操作をE2Eで検証し、種目選択プレビューは390pxで横スクロール可能な全セット表示を確認する。
- 未解決事項・次のアクション: 記録画面のチェック成功後、実装修正だけをPR #194へ追加する。種目選択は採用案確定後に別途実装する。

## 2026-09-24 デザインルール準拠の種目選択案を作成

- 変更内容・目的: ユーザー指定により、記録入力の温度感とデザインルールを明示的に適用した種目選択の3案を作成した。白地・細い罫線・4/8/12/16pxの余白・48px操作領域を基準にし、角丸カードとカプセル型タブを使わない。赤は部位の選択状態と終了操作へ限定し、全セットの今日メニューは横一列で確認できる。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v4.html、progress.md。実アプリ、API、DB、種目の並び替え規則は変更しない。
- 検証: 390px幅、ライト/ダーク、胸・腕の複数選択、全8部位、今日メニューの横スクロール、横はみ出しなしを確認する。
- 未解決事項・次のアクション: ユーザーが採用案を選択後、実アプリの種目選択へ同じ構成を実装し、部位和集合・最近使用順・先週比較のデータ規則をテストとともに確定する。

## 2026-09-24 トレーニング全体導線のプレビューを追加

- 変更内容・目的: ユーザー指摘により、種目選択を単体画面として扱うのをやめ、開始、部位・種目選択、セット入力、終了確認、結果までを操作できる一つのプレビューへまとめた。種目選択へ戻っても部位・今日のメニュー・保存済みセットを保持する前提とし、種目追加は選択画面を離れない小さなシートへ分離した。
- 影響範囲・関連ファイル: frontend/public/previews/training-flow-options.html、progress.md。実アプリ、API、DB、端末下書きの処理は変更しない。
- 検証: 390px幅で開始から結果まで遷移でき、終了確認から入力へ戻れること、部位複数選択と種目追加シートを確認する。
- 未解決事項・次のアクション: ユーザーが導線と種目選択案を選んでから、実アプリの画面遷移、部位フィルター、今日のメニュー集計、種目追加シートを実装する。

## 2026-09-24 種目選択プレビューの前提を修正

- 変更内容・目的: ユーザー確認で、先行プレビューが確定済みの記録入力画面まで仮に作り替えており、実装前提として不適切だったことを認識した。種目選択だけに範囲を戻し、部位を全8件一行に配置、選択は赤い文字と下線だけ、今日行ったセットは種目ごとの縦積みに固定した3案を作成した。
- 影響範囲・関連ファイル: task.md、frontend/public/previews/training-selection-options-v5.html、progress.md。実アプリ、記録入力、終了確認、結果、API、DBは変更しない。
- 検証: 実装へ移す前提で、既存の記録入力の白地・細い罫線・赤い主操作・48px以上の操作領域を参照して静的プレビューを作成した。部位に追加の記号や枠を置かず、セット値を横並びにしない。
- 未解決事項・次のアクション: ユーザーが今日のメニュー、部位、候補、総負荷の上下順を決定してから、実データの集計規則と種目追加導線を確定し、既存入力画面を変更せずに実装する。

## 2026-09-24 今日のメニューと週次総負荷の表示案を追加

- 変更内容・目的: ユーザー提案により、今日のメニューは初期状態で全セットを占有しない展開行へ変更した。開いた内容は種目ごと・セットごとに必ず縦に並ぶ。総負荷は当日の値ではなく今週の値とし、先週を基準にした棒、曜日ごとの棒、小さい円の3案を作成した。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v6.html、progress.md。実アプリ、既存の記録入力・終了・結果画面、API、DBは変更しない。
- 検証: 展開前後でセット値が横並びにならないこと、部位が一行で選択を色と下線のみで表すことをプレビューの構造で確認した。
- 未解決事項・次のアクション: ユーザーが週次比較を採用する可視化と、基準を先週・直近4週平均などのどれにするかを決めてから実装する。

## 2026-09-24 種目選択のLIVE表示案を追加

- 変更内容・目的: ユーザー提案により、採用候補の週次棒グラフへ、現在同じグループで記録している人と最新セットを短く見せるLIVE行を組み合わせた。今日のメニューの展開入口は「＋」から「…」へ変更した。LIVEの位置は、見出し直下・週次棒の横・種目候補の後ろの3案とした。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v7.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: 既存の`GET /groups/{group_id}/activity`がLIVE状態と最新セット（表示名・種目・重量・回数）を返すことを確認した。プレビューではLIVEを1行または固定小領域に収め、更新時も種目候補や入力画面の高さが変わらない構成にした。
- 未解決事項・次のアクション: 選択中グループのLIVEを使うか全所属グループを集約するか、更新周期と詳細パネルの内容をユーザーと決めてから実装する。

## 2026-09-24 総負荷を外したアイコン主体のLIVE案を追加

- 変更内容・目的: ユーザー提案により、種目一覧の入口と今週の総負荷を画面から外し、既存プロフィール画像を主に使うLIVE案を作成した。最新の一人を一行で見せる案、LIVE中の人をアイコンで選ぶ案、種目候補の後ろに置く案を比較できるようにした。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v8.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: 既存`Avatar`コンポーネントがプロフィール画像の取得失敗時に頭文字へ戻り、LIVE時に赤い輪と点を表示することを確認した。プレビュー内の円形サンプルは、この既存コンポーネントへの置換位置を示すだけで、画像保存や通信の変更は含まない。
- 未解決事項・次のアクション: ユーザーがLIVEの位置と、複数人を常時アイコンで見せるかを決めた後、選択中グループの`GroupActivity`を既存の更新規則で取得し、詳細シートの内容を実装する。

## 2026-09-24 グループ切替とアイコン列のLIVE案を追加

- 変更内容・目的: ユーザーがA案を基準に採用したため、最新セットを一人分だけ一行で見せる構造を維持したまま、グループごとの切替を比較できる3案を作成した。LIVE行を横スワイプする案、グループ内のLIVE中メンバーをアイコン列から選ぶ案、グループ名の横列で切り替える案である。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v9.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: 各案で、LIVEの最新セットは一行、今日のメニューのセットは縦積み、部位は一行という確定済みの前提を維持した。既存APIはグループ単位のLIVE状態・フィードを返すため、選択中グループだけを必要時に取得する構成で実装できる。
- 未解決事項・次のアクション: ユーザーがグループ切替方式と、アイコンを常設する人数・詳細の開き方を決めてから実装する。

## 2026-09-24 今日の仲間・記録カード・スタンプの導線案を追加

- 変更内容・目的: ユーザー指定により、LIVEだけでなく今日すでに終了した人も含むアイコン列を、種目選択の上へ置く案に切り替えた。初期表示は全所属グループ、グループ単位の表示へ切替可能とし、アイコン選択後の記録カードを行内・下シート・小さい要約の3案で比較する。カードには既存の直接スタンプ操作を置く。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v10.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: 既存のグループフィードがユーザーごとの最新記録、記録詳細、グループ単位スタンプを持つことを確認した。完了済みの「今日の記録」に限定するためには、現在の最新記録取得に当日条件を加える実装が必要であることも確認した。
- 未解決事項・次のアクション: ユーザーが記録カードを開く位置を決める。採用後に、全所属グループの重複ユーザー処理・今日の記録だけを返すAPI規則・スタンプを送るグループの決定規則を仕様化する。

## 2026-09-24 記録カードを下シートへ採用し、候補の前回値を日数へ変更

- 変更内容・目的: ユーザーがB案（アイコン選択後に下から今日の全セットカードを開く）を採用した。種目候補で前回の重量・回数を一セットだけ見せる表示は比較に役立たないため、プレビューでは「3日前」「6日前」のような前回からの日数へ変更した。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v10.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: 既存の種目コンテキストが`previous.performed_on`を返すことを確認した。候補ごとの日数表示は、この日付からクライアント側で算出でき、新しい保存項目を必要としない。
- 未解決事項・次のアクション: 採用後、候補の上位数件へ現在行っている種目コンテキスト取得を広げ、当日との差分を表示する。日付がない種目の表示文言も実装時に決める。

## 2026-09-24 種目選択の段階的読み込み案を追加

- 変更内容・目的: ユーザー指摘により、前回からの日数・全グループの仲間・記録カードの通信待ちが、種目選びの妨げにならない表示を試作した。最初に部位と候補を操作可能にし、未取得部分だけを高さ固定の薄いプレースホルダーで示す。アイコン選択時は下シートを即時表示し、詳細とスタンプをシート内で置き換える。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-loading-v1.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: 既存`useResource`は保持済みデータを再取得中も返せ、種目コンテキストのキャッシュは最大2件ずつ取得して選択中を優先することを確認した。表示では全画面スピナーとロード中の説明文を置かず、各領域の高さを固定する。
- 未解決事項・次のアクション: 採用後、開始前の候補・所属グループの先読み、選択中グループ以外の活動情報の遅延取得、下シートの詳細・スタンプの独立したプレースホルダーを実装し、低速通信E2Eで位置・操作の不変性を検証する。

## 2026-09-24 候補の前回日数を10日で丸める

- 変更内容・目的: ユーザー指定により、種目候補の前回日数は細かな古い日付を出さず、10日を超えた場合は「10日以上前」と表示する前提へ変更した。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-options-v10.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: プレビューで短い日数の「3日前」と、上限表現の「10日以上前」を確認できるようにした。
- 未解決事項・次のアクション: 実装時に`Math.min`などで日数を丸め、前回記録なしの場合の表示を決定する。

## 2026-09-24 採用B案へ終了操作と今日のトレーニングを復帰

- 変更内容・目的: ユーザー指摘により、採用したB案の種目選択に、終了操作と今日のトレーニングの確認入口が欠けていたため復帰した。右上の「今日のトレーニング終了」は既存の終了確認へ、今日のトレーニングは「…」の展開行で全セットを縦に確認できる。仲間アイコンは下シートの全セットカードを開く。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v11.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: プレビュー上でアイコン→記録カード、右上の終了→既存と同じ「まだ続ける」「今日のトレーニング終了」の確認、今日のトレーニング展開を操作できるようにした。
- 未解決事項・次のアクション: 実装時は種目候補・所属グループを開始前に先読みし、全グループの当日アクティビティは1回の集約取得にする。アイコン・候補・カード詳細を独立して更新し、低速通信でも主操作を無効化しない。

## 2026-09-24 今日のトレーニング要約へ総負荷と追加入口を追加

- 変更内容・目的: ユーザー指定により、今日のトレーニングの「2種目・5セット」と同じ行へ総負荷を追加した。下部の追加入口は記号だけでなく「＋ 種目を追加」と明示した。終了操作は新規の見た目を作らず、確定済みの記録画面と同じ`finish-training`の赤いボタンを使う前提で比較した。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v12.html、progress.md。実アプリ、記録入力、終了確認、API、DBは変更しない。
- 検証: セッションが保持している今回の全セットから総負荷を即時計算できるため、総負荷自体に追加通信・プレースホルダーが不要であることを確認した。3案すべてで終了ボタンの高さ・赤い塗り・文言を現行の確定仕様に合わせた。
- 未解決事項・次のアクション: ユーザーが要約行の表記を選んでから、採用B案の実装と低速通信E2Eを進める。

## 2026-09-24 種目選択B案の要約表記と追加時の部位初期値を採用

- 変更内容・目的: ユーザーが今日のトレーニング要約のA案「2種目　5セット　3,240kg」を採用した。さらに、部位で絞り込んだ流れを保つため、「＋ 種目を追加」で開く追加フォームは、選択部位が1件なら主部位へ初期値を入れ、複数・未選択では主部位を未選択にする仕様へ見直した。
- 影響範囲・関連ファイル: task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 現行の保存では主部位が必須であることを確認した。複数選択時はフォーム上では未選択にし、保存前に利用者が決めるため、保存形式を変えずに主部位の誤推定を防げる。追加フォーム内では既存どおり部位を変更可能とする。
- 未解決事項・次のアクション: 仲間アイコンの全所属グループ集約と当日記録条件、スタンプを送るグループ選択規則、前回実施日がない候補の表示を決めてから、採用B案を実装する。

## 2026-09-24 追加時の主部位と仲間カードの補助情報を比較

- 変更内容・目的: ユーザー指定により、単一部位選択時の主部位初期値、複数部位選択時の未選択フォーム、追加してもよい補助情報をプレビュー化した。補助情報は種目候補を増やさず、今日の仲間カードの「たった今」と既存のBEST表示に限定する。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v13.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 現行の`BodyPartFields`が主部位必須・補助部位任意であること、既存フィードが更新時刻とBEST情報を返すことを確認した。
- 未解決事項・次のアクション: ユーザーが複数選択時の追加フォームと、仲間カードの経過時間・BESTの採用可否を決めてから、フォーム初期値とカード表示を実装する。

## 2026-09-24 主部位優先の候補と高さ不変のLIVE案を追加

- 変更内容・目的: ユーザー指定により、部位で絞る候補は主部位が一致する種目を先に出し、補助部位が一致する種目を下の「補助部位」区切りへ置く案にした。LIVE感は画面セクションの高さや並びを変えず、既存の赤いLIVE輪、新着セットの一時強調、新着後の小さい点の3案で比較する。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v14.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 既存の種目分類が主部位・補助部位を持つこと、既存のグループフィードが更新差分を4秒だけ強調する処理を持つことを確認した。各案の仲間アイコン列は75px、種目行は57pxで固定し、状態更新で高さを増やさない。
- 未解決事項・次のアクション: ユーザーがLIVEの強調案と補助部位区切りの表記を選んでから、部位フィルターの並び順・当日フィードの差分表示・動作軽減設定を実装する。

## 2026-09-24 補助部位を省く候補とBESTの炎を比較

- 変更内容・目的: ユーザー指摘により、補助部位を区切って常時出す案を外し、主部位だけの最短候補、補助部位を下へ続ける案、BESTを記録カードだけで見せる案を比較した。速度のため、初期表示は主部位のみを優先する。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v15.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 主・補助の分類は既存種目候補に含まれるため、主部位だけの表示は追加通信を必要としない。既存の記録・グループフィードはBESTの重量/RM判定を返すため、炎の追加判定も不要であることを確認した。
- 未解決事項・次のアクション: ユーザーが主部位だけにするか、補助部位を続けて出すか、炎をアイコンかカードのどちらに置くかを決めてから実装する。

## 2026-09-24 主部位のみと最高記録の炎を比較

- 変更内容・目的: ユーザーが主部位のみの候補表示を採用した。さらに、炎は最高重量または推定1RMを更新したセットだけに限定し、仲間アイコンと、アイコンから開く記録カードでの見え方を3案で比較した。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v16.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 既存のグループフィードと記録詳細が最高重量・最高RMの個別判定を返し、既存の記録画面も同じ条件でBESTを表示することを確認した。炎のための追加判定・保存・通信は不要である。
- 未解決事項・次のアクション: ユーザーがアイコンとカードでのBEST表示を選んでから、採用B案を実装する。

## 2026-09-24 最高記録表示を最小化

- 変更内容・目的: ユーザー確認により、BEST専用の行、背景、文言は情報量を増やすため不採用とした。最高重量または推定1RMを更新した場合だけ、仲間アイコン右上へ小さな炎を添え、記録カードでは該当セットの既存値を赤くして横に炎を添える最小表示案へ変更した。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v17.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: アイコン列・カード行ともに既存の高さを変えず、BESTでない行へ表示を追加しない構造を確認した。
- 未解決事項・次のアクション: ユーザー確認後、既存のBEST判定をそのまま使って採用B案を実装する。

## 2026-09-24 全セットRMと意欲が出るBESTアイコン案を追加

- 変更内容・目的: ユーザー指定により、仲間の記録カードは全セットへ推定1RMを表示し、総負荷は上部の今日要約とカードの集計行へ常時置くことにした。BESTの炎は高さを変えず、アイコン右上での小さい炎・少し大きく光る炎・丸印付きの炎を比較した。カード内は更新セットの値を赤くし横に炎を置く。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-b-v18.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 推定1RMは既存の重量・回数から算出でき、カードの全セットへ追加通信なしで表示できる。総負荷も現在保持中のセットから即時計算でき、プレースホルダーは不要である。
- 未解決事項・次のアクション: ユーザーが炎のアイコン案を選んでから、採用B案の実装と低速通信E2Eを進める。

## 2026-09-24 小さい炎と全スタンプを採用候補へ反映

- 変更内容・目的: ユーザーが最高記録アイコンのA案（小さい炎）を選んだ。記録カードのスタンプは、＋から開く方式ではなく、現在の6種類（💪・🔥・👎・🤩・👏・👺）を常設で並べる案を追加した。
- 影響範囲・関連ファイル: frontend/public/previews/training-selection-adopted.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 現行の直接スタンプ候補が6種類であること、同一ユーザー・同一記録・同一グループ・同一種類を一意にするDB制約とトグル式の送信処理があることを確認した。したがって、現行仕様では種類ごとに1回ずつ送信・取消でき、同じ種類の連続送信はできない。
- 未解決事項・次のアクション: ユーザーの再確認により、同じ種類は1回・再タップで取消する現行仕様を維持する。カードへ常設する6種類の表示と既存トグル操作を接続する。

## 2026-09-24 BESTの仲間アイコン候補を比較

- 変更内容・目的: ユーザー指定により、記録カードや他の画面は変えず、仲間アイコン右上に置くBEST印だけを比較した。炎、稲妻、きらめき、トロフィー、上向き矢印の5候補で、LIVEの赤い輪・右下点は共通で維持する。
- 影響範囲・関連ファイル: frontend/public/previews/best-avatar-icon-options.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: すべてアイコンの右上だけを使い、アイコン列の幅・高さ・LIVE状態表示を変えない構成を確認した。
- 未解決事項・次のアクション: ユーザーがBESTアイコンを選んでから、既存の最高重量・最高RM判定へ表示を接続する。スタンプは現行の種類ごと1回・再タップ取消を維持する。

## 2026-09-24 図を増やさないBESTアイコン強調を比較

- 変更内容・目的: ユーザー指摘により、BESTの右上図は炎を基準とし、図以外で目立たせる方法を追加した。オレンジの外周、淡い発光、名前下の短い線を、LIVEの赤い輪・右下点を維持したまま比較する。
- 影響範囲・関連ファイル: frontend/public/previews/best-avatar-emphasis-options.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: すべてアイコン列の幅・高さを変えず、最高記録のある人だけに適用できる。LIVEは右下点として独立して残る。
- 未解決事項・次のアクション: ユーザーがBESTの強調方法を選んでから、既存の最高重量・最高RM判定へ表示を接続する。

## 2026-09-25 BESTアイコンを炎へ統一

- 変更内容・目的: ユーザーがBESTの仲間アイコンは炎を採用した。トレーニング詳細の既存炎と同じ意味・見た目に揃え、最高重量または推定1RMを更新した場合だけアイコン右上と記録カードの該当セットへ付ける。
- 影響範囲・関連ファイル: task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 炎は既存の最高記録判定とトレーニング詳細で使う意味に揃え、LIVEの赤い輪・右下点とは独立した状態として記録した。
- 未解決事項・次のアクション: 全所属グループの当日記録集約、スタンプ送信先、前回記録なしの表示を決めてから、採用B案を実装する。

## 2026-09-25 スタンプ詳細シートを比較

- 変更内容・目的: ユーザー指定により、記録カードのスタンプ横にある`…`から開くリアクション詳細を再設計した。対象の記録者と共有グループを固定し、スタンプ別、人別、時系列の3案を比較する。
- 影響範囲・関連ファイル: frontend/public/previews/stamp-details-options.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 現行のスタンプ詳細APIが送信者・種類・作成時刻・グループを返すことを確認した。いずれの案もこのデータだけで表示できる。
- 未解決事項・次のアクション: ユーザーが詳細の並び方を選んでから、既存のリアクション詳細シートを採用構成へ更新する。

## 2026-09-25 種目選択の設計判断とスタンプ詳細A案を確定

- 変更内容・目的: ユーザーがスタンプ詳細はA案（スタンプごとに送信者をまとめる）を選択した。対象は記録者名ではなく日付と共有グループとし、件数・人数の要約、スタンプの日本語名・人数表記を外した。今後の検討で同じ判断を繰り返さないよう、種目選択・仲間カード・スタンプ詳細に加え、これまで確定した記録入力・メモ・セット操作・削除・終了のルールも `docs/design/training-session.md` に集約した。画面別資料は `docs/design/` へ置くことにし、`AGENTS.md` には設計判断を必ず文書化・事前参照する運用を追加した。
- 影響範囲・関連ファイル: AGENTS.md、docs/README.md、docs/design/README.md、docs/design/training-session.md、frontend/public/previews/stamp-details-selected.html、task.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 採用プレビューで、スタンプ行が絵文字と送信者アイコンだけであり、日付・グループ以外の対象情報、件数・人数、日本語の意味づけがないことを確認した。文書リンクと `git diff --check` は変更後に確認する。静的プレビューと文書のみの変更のため、アプリテストは実行しない。
- 未解決事項・次のアクション: 全所属グループの当日記録集約、スタンプ送信先、前回記録なしの表示規則を決めてから、設計資料を参照して実装・テストへ進む。

## 2026-09-25 スタンプ詳細の対象情報を記録要約へ変更

- 変更内容・目的: ユーザー確認により、採用したスタンプ詳細A案の余白を埋める対象情報として、記録者名ではなく日付・種目数・セット数・総負荷を一行で置くことにした。共有グループは次の行へ短く残し、最高記録を更新した日だけ右端に小さな炎を添える。シート名は `リアクション` ではなく `スタンプ` に統一した。
- 影響範囲・関連ファイル: docs/design/training-session.md、frontend/public/previews/stamp-details-selected.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: プレビューで、件数・人数やスタンプの日本語名を復活させずに、何へのスタンプかを日付・トレーニング量で把握できることを確認する。静的プレビューと資料のみの変更のため、アプリテストは実行しない。
- 未解決事項・次のアクション: 全所属グループの当日記録集約、スタンプ送信先、前回記録なしの表示規則を決めてから、設計資料を参照して実装・テストへ進む。

## 2026-09-25 採用デザインを固定プレビューと紐付け

- 変更内容・目的: ユーザー指定により、設計資料の採用事項を比較途中の版番号付きプレビューではなく、固定名の採用プレビューと画面・状態ごとに紐付けた。種目選択・仲間カードは `training-selection-adopted.html`、スタンプ詳細は `stamp-details-selected.html`、確定済みの記録入力・終了は `header-selected.html` を実装・レビュー時の照合元にする。今後の運用として、採用プレビューを固定名で残して設計資料から参照する規則を `AGENTS.md` に追加した。
- 影響範囲・関連ファイル: AGENTS.md、docs/design/training-session.md、frontend/public/previews/training-selection-adopted.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: 固定プレビューへのリンク、画面・状態ごとの照合項目、390pxおよび320/430pxでの確認要件を設計資料に記載した。静的プレビューと資料のみの変更のため、アプリテストは実行しない。
- 未解決事項・次のアクション: 全所属グループの当日記録集約、スタンプ送信先、前回記録なしの表示規則を決めてから、固定プレビューと照合して実装・テストへ進む。

## 2026-09-25 グループ切替と空スタンプの表示規則を確定

- 変更内容・目的: ユーザー指定により、仲間の初期表示は全所属グループの `すべて` とし、グループを選んだときだけ対象の記録へ切り替えることを確定した。スタンプ詳細は0件なら説明や空行を出さず、送信者の識別はアイコンだけにして名前表示・名前一覧への遷移を設けない。
- 影響範囲・関連ファイル: docs/design/training-session.md、progress.md。実アプリ、API、DBは変更しない。
- 検証: 既存の採用プレビューが `すべて` の初期状態とアイコン主体の構成を満たすことを確認した。空スタンプは描画する行がない状態として実装でき、追加通信や画面高さを必要としない。資料のみの変更のため、アプリテストは実行しない。
- 未解決事項・次のアクション: 全所属グループの当日記録集約、`すべて` のままスタンプを送る場合の対象グループ、前回記録なしの表示規則を決めてから、固定プレビューと照合して実装・テストへ進む。

## 2026-09-25 スタンプ詳細のグループ切替と閲覧範囲を明示

- 変更内容・目的: ユーザー指摘により、スタンプ詳細の採用プレビューに `すべて`・所属グループの切替行を追加した。`すべて` は閲覧者と記録者の共通所属グループだけを集約し、閲覧者が入っていない別グループのスタンプは無視する閲覧範囲を明記した。
- 影響範囲・関連ファイル: docs/design/training-session.md、frontend/public/previews/stamp-details-selected.html、progress.md。実アプリ、API、DBは変更しない。
- 検証: プレビューで日付・記録要約の直下にグループ切替があり、初期選択が `すべて`、選択状態が赤い下線だけで分かることを確認する。資料・静的プレビューのみの変更のため、アプリテストは実行しない。
- 未解決事項・次のアクション: 全所属グループの当日記録集約、`すべて` のままスタンプを送る場合の対象グループ、前回記録なしの表示規則を決めてから、固定プレビューと照合して実装・テストへ進む。

## 2026-09-25 種目選択と当日仲間の高速取得を実装

- 変更内容・目的: 採用プレビューを基準に、開始後の種目選択を検索欄から主部位8件の複数選択へ置き換えた。今日のトレーニングは種目数・セット数・総負荷の展開行にし、終了は確定済みの赤い `今日のトレーニング終了` を使う。仲間は `すべて` を初期値とする横スクロール可能なグループ切替とアイコン列にし、BESTを小さな炎で表示する。候補は主部位だけをブラウザ内で即時に絞り、仲間一覧は当日分を一つのHTTP応答で取得、記録詳細・直接スタンプはアイコンを押した後だけ取得するようにした。追加フォームは部位を一つだけ選択中なら、その主部位を初期値として開く。
- 影響範囲・関連ファイル: backend/app/api/routes/sessions.py、backend/app/infrastructure/sessions.py、backend/app/schemas/session.py、backend/tests/test_session_performance.py、frontend/src/lib/api.ts、frontend/src/features/exercises/exercise-catalog.tsx、frontend/src/features/session/session-screen.tsx、frontend/src/features/session/use-exercise-context.ts、frontend/src/features/stamps/stamp-control.tsx、frontend/src/app/v2.css、frontend/tests/e2e/mock-training.ts、frontend/tests/e2e/training-selection.spec.ts、docs/design/training-session.md、task.md、progress.md。migration不要。記録入力・終了確認・結果画面の保存規則は変更しない。
- 検証: `bun run lint`、`bun run typecheck`、`bun run build`、種目選択E2E、既存session-flow E2Eを実行した。backendの`ruff check`は成功した。DB統合テストはこの環境に`TEST_DATABASE_URL`がなく5件すべてskipとなり、実DBでの集約API検証は未実施。
- 未解決事項・次のアクション: 採用プレビューと実機幅で詳細シート・スタンプ詳細の見た目を照合する。`すべて`のまま直接スタンプを送る際の対象グループ、前回記録なしの補助表示、単一部位選択時の種目追加フォーム初期値を決めて実装する。
## 2026-09-25 WSL/Supabase起動障害の復旧と再発防止

- 変更内容・目的: Docker Desktopのポート公開自体は成功しているにもかかわらず、WSL内のSupabase CLIが `127.0.0.1:59322` のPostgreSQLへ接続できず起動を停止する障害を調査した。Windowsの `%USERPROFILE%\\.wslconfig` をバックアップした上で、`networkingMode=mirrored` から標準の `networkingMode=nat` と `localhostForwarding=true` に戻し、WSLを再起動した。再発時に同じ診断・復旧を行えるよう、AGENTS.mdへ手順と禁止事項を追加した。
- 影響範囲・関連ファイル: WindowsのWSLネットワーク設定（バックアップ済み）、AGENTS.md、progress.md。アプリコード、DBスキーマ、認証仕様は変更しない。既存のGO TORE DB/Storageは削除前に `.local-backups/` へバックアップ済みで、今回の起動は新規ローカルDBとしてmigrationとseedを適用する。
- 検証: 同じSupabase PostgreSQLイメージを `59322` に一時公開して、コンテナ内の `pg_isready` とWSLからの `nc -zv 127.0.0.1 59322` が成功することを確認後、一時コンテナを削除した。`make db-start` はGO TOREの全Supabaseコンテナを起動し、db/auth/kongを含むhealth check成功を確認した。資料のみの変更のため、アプリのテストは実行していない。
- 未解決事項・次のアクション: backend/frontendを起動し、PCと同一LANのスマホからアクセスできること、Authの実ログインを順に確認する。新規ローカルDBのため、以前のローカルアカウントはバックアップから復元しない限り存在しない。
## 2026-09-25 ローカル閲覧用サンプルデータを作成

- 変更内容・目的: ユーザー依頼により、再作成したローカルSupabase上のテストユーザーを所有者として、閲覧用の「朝トレ部」「週末ワークアウト」、メンバー3人、完了済みトレーニング3件を作成した。DBへ直接書き込まず、通常画面と同じ認証・API経路を利用した。
- 影響範囲・関連ファイル: ローカルSupabaseの認証・業務データのみ。アプリコード、migration、公開環境は変更しない。progress.md。
- 検証: テストユーザーのパスワードログイン後、`GET /api/groups` でグループ2件、`GET /api/groups/today-activity` でアクティビティ対象2グループを取得できることを確認した。
- 未解決事項・次のアクション: データはローカル開発用であり、`make db-reset` やSupabaseボリューム削除で失われる。必要なら画面から記録やメンバーを追加して表示を確認する。
## 2026-09-25 終了操作の統一と種目一覧の再設計方針

- 変更内容・目的: ユーザー指摘により、種目選択と記録入力で同じ「今日のトレーニング終了」が別の見た目・位置に見えないよう、共通の `session-header` と終了ボタンのスタイルへ統一した。右端、48pxのヘッダー、40pxの赤い一行ボタン、文字サイズ、角丸なしを両画面で共通にした。種目の追加・編集・削除を行う一覧は設定専用としてプレビューから全面再設計し、記録中に管理画面を混ぜない方針を設計資料とtaskへ記録した。
- 影響範囲・関連ファイル: frontend/src/features/session/session-screen.tsx、frontend/src/app/v2.css、frontend/tests/e2e/training-selection.spec.ts、docs/design/training-session.md、task.md、progress.md。終了の保存・確認処理、種目候補の選択、API・DBは変更しない。
- 検証: `bun run lint` 成功。390pxブラウザE2Eで、候補の部位絞り込みを維持しつつ、種目選択と入力の終了ボタンの幅・高さ・背景色・文字色・角丸が一致することを確認した。候補モックを初期表示後に差し替える既存テストの不安定さを、再読込で解消した。
- 未解決事項・次のアクション: 種目一覧の採用プレビューを作成し、設定専用の追加・分類変更・削除導線を合意してから、記録中の現行一覧シートを廃止する。現時点では一覧の操作仕様を変更しない。
## 2026-09-25 LAN公開とUI再利用の運用ルールを追加

- 変更内容・目的: スマホからの開発画面接続を調査し、WSLの画面・Windowsのportproxy・ローカルAuthは到達する一方、Windows Defender FirewallにTCP 3001の受信許可がないことを確認した。WSL IPが変わる可能性を含めたLAN確認手順をAGENTS.mdへ追加した。あわせて、同じ役割・優先度のボタンや一覧行は共通部品・共通スタイルを再利用し、統一感と操作の簡便さを目指すデザインルールをdesign-system.mdへ追加した。
- 影響範囲・関連ファイル: AGENTS.md、docs/design-system.md、progress.md。アプリの業務仕様・API・DBは変更しない。
- 検証: WSLの3001待受、portproxyのWSL中継先、WindowsのLAN IPでの画面・Authの200応答を確認した。Windowsファイアウォールの既存GO TORE規則には3001が含まれないことを確認した。
- 未解決事項・次のアクション: 管理者権限でWindows Defender FirewallのTCP 3001受信許可（`GOTORE3001`）を追加し、規則がTCP 3001を許可することを確認した。実機スマホから `http://192.168.0.28:3001` を確認する。種目選択は採用済みプレビューの共通ボタン・一覧行を起点に、管理用種目一覧と分離して実装を続ける。採用プレビューと設計本文で「今日のトレーニング」と仲間一覧の順が矛盾するため、実装前に順を確認する。
## 2026-09-25 種目選択を採用プレビューの順へ統一

- 変更内容・目的: ユーザー指定により、種目選択を採用プレビューの順番へ修正した。見出しと終了操作の下にグループ切替・仲間アイコンを置き、その下に閉じた「今日のトレーニング」、部位、候補行を続ける。設計本文もプレビューと同じ順へ直した。終了操作は種目選択・記録入力で同じ共通ヘッダーと、8px角丸の赤いボタンを使う。
- 影響範囲・関連ファイル: frontend/src/features/session/session-screen.tsx、frontend/src/app/v2.css、frontend/tests/e2e/training-selection.spec.ts、docs/design/training-session.md、progress.md。保存・共有・候補データ・終了処理は変更しない。
- 検証: `bun run lint` と `bunx playwright test tests/e2e/training-selection.spec.ts` が成功。390pxで仲間一覧が今日のトレーニングより上にあり、選択・入力の終了ボタンの幅・高さ・背景色・文字色・角丸が一致することを自動確認した。
- 未解決事項・次のアクション: 設定専用の種目管理一覧を採用プレビューから作り直し、記録中の管理シートを廃止する。管理用の追加・編集・削除テストを新しい導線へ合わせる。
## 2026-09-25 設定専用の種目追加画面と候補共有を実装

- 変更内容・目的: 設定の種目管理で、一覧の途中に追加フォームを表示する構成をやめた。`種目を追加` は種目名・主な部位・任意の補助部位・追加・キャンセルだけを表示する独立画面にし、既存の主/補助ボタンと入力部品を再利用した。設定用カタログと記録中の候補が別々のキャッシュを持ち、追加直後に選べない不具合を修正して、一つのカタログ状態を共有した。
- 影響範囲・関連ファイル: frontend/src/features/exercises/exercise-catalog.tsx、use-exercise-catalog.ts、session/session-screen.tsx、v2/workspace.tsx、v2.css、frontend/tests/e2e/exercise-management.spec.ts、docs/exercise-body-parts.md、progress.md。種目API、DB、保存済み履歴、共有範囲は変更しない。
- 検証: `bun run lint` 成功。`bunx playwright test tests/e2e/exercise-management.spec.ts` の3件が成功し、追加画面では検索一覧を表示しないこと、追加した候補が設定から記録へ移動後すぐ選択一覧へ出ること、編集・削除時の再取得/競合保護を確認した。
- 未解決事項・次のアクション: 記録中に残る現行の種目管理シートを、設定専用の作り直しと同じ変更で廃止する。実機スマホで画面の追加・記録選択・LANアクセスを確認する。

## 2026-09-25 採用ホームと黒ヘッダーの実装を開始

- 変更内容: ログイン後の共通ヘッダーを高さ48pxの黒地と白い `E-GOTORE` へ統一した。ホームへ全所属グループの `LIVE / 今日 / セット / 総負荷` 4列要約、近接したスワイプ式グループカード、記録単位で全セットを表示する時系列を実装した。フィードAPIは利用者ごとの最新1件ではなく更新順の記録を最大50件返し、要約へ総負荷を追加した。`START / RESUME` は長押しで移動し、左右端へ吸着して利用者・端末別に位置を保存する共通部品へ分離した。種目選択と入力の黒ヘッダーはロゴと右端の終了操作を共通化し、入力中の種目名は直下へ移した。
- 目的: 採用プレビューの情報階層を実データへ反映し、ホーム以外でも同じ記録開始操作とヘッダーを使えるようにするため。
- 影響範囲: ホーム、グループ活動フィード、共有記録の先読み、記録中ヘッダー、開始・再開操作、活動APIスキーマ。DB migrationと共有権限は変更しない。
- 関連ファイル: `frontend/src/features/v2/community.tsx`、`floating-training.tsx`、`use-shared-workout-details.ts`、`workspace.tsx`、`session-screen.tsx`、`frontend/src/app/v2.css`、`frontend/src/lib/api.ts`、`backend/app/infrastructure/sessions.py`、`backend/app/schemas/session.py`、`docs/design/home-screen.md`、`docs/design-system.md`、`docs/design/training-session.md`、`task.md`。
- 検証: frontend lint、TypeScript、単体テスト99件、production buildは成功。backend ruffは成功。DB依存テストは3件成功・13件skip（専用DB未指定）。対象E2EはSTART/RESUME・黒ヘッダー・種目選択7件とホーム/グループ6件の合計13件が成功した。320/390/430pxの画像を確認し、4列とグループカードの間に残っていた補助導線をタイムライン後へ移した。
- 未解決事項: 専用DBで複数グループ共有時の重複集計を確認する。入力フォームのサイズ変更は既存Issue #110へ追加要望をコメントし、ホーム実装から分離した。
- 次のアクション: テストアカウントで実データの全セット・スタンプを確認し、必要ならカード内の文字間隔だけを調整する。

## 2026-09-25 ホームの実機指摘を採用プレビューへ再調整

- 変更内容: LIVEの点を真円へ固定し、ホームのグループカードを採用プレビューの濃色へ戻した。時系列は日付・時刻を最上部へ移し、総負荷・セット数・種目表を小さな白いカードへまとめ、LIVE時の左赤線を削除した。ホーム、共有履歴、トレーニング中の仲間詳細は同じスタンプ部品で6種類を最初から表示し、選択・詳細の読み上げ名から日本語の意味づけを外した。トレーニング中の黒ヘッダーは上余白をなくし、高さ固定・上端固定とした。浮遊STARTの長押し開始時間は280msから140msへ短縮した。
- 目的: スマホ確認で見つかった採用プレビューとの差と、同じ目的のスタンプ操作が画面ごとに異なる問題、ヘッダーの位置ずれを解消するため。
- 影響範囲: ホームの要約・グループ入口・時系列、共有記録のスタンプ、トレーニング中ヘッダー、START/RESUMEの移動開始。API、DB、共有権限は変更しない。未合意のグループ画面の情報構成は変更していない。
- 関連ファイル: `frontend/src/app/v2.css`、`frontend/src/features/stamps/stamp-control.tsx`、`frontend/src/features/training/record-list.tsx`、`frontend/src/features/v2/community.tsx`、`history-browser.tsx`、`shared-workout-detail.tsx`、`floating-training.tsx`、関連E2E・単体テスト、`docs/design/home-screen.md`、`docs/design/training-session.md`、`docs/stamps.md`。
- 検証: frontend lint、TypeScript、単体テスト99件は成功。対象E2Eはホーム・START・ヘッダー・種目選択7件、常設スタンプ1件、現在の共有記録2件が成功した。320/390/430pxの横収まり、濃色カード、真円LIVE、日付の位置、常設6スタンプ、ヘッダー上端、START移動を確認した。旧い「カードを押して詳細シートを開く」前提だった共有記録E2Eも、現在の全セット直接表示と画面外での取得停止を検証する内容へ更新した。LAN経由の画面 `:3001`、Auth `:59321`、API `:8000` はすべてHTTP 200を確認した。
- 未解決事項: グループ画面そのものは未合意のため、別途プレビューで決めるまで変更しない。
- 次のアクション: スマホで更新後の画面を再確認する。未合意のグループ画面は、別途プレビューで決定してから実装する。

## 2026-09-26 全グループ混在表示の選択方法を比較

- 変更内容: 採用済みホームを土台に、全所属グループの記録を混ぜた `すべて` を初期表示とする4案の操作可能な静的プレビューを追加した。Aは4列数値全体、Bは固定ピル、Cは半幅の白いパネル、Dはタイムライン右上メニューから `すべて` へ戻る。個別グループカードは全案で横スワイプと長押し並べ替えを維持し、カード選択による絞り込みと詳細矢印を分離した。Dだけはカードを閲覧・並べ替え用とし、絞り込みを右上メニューへ集約した。
- 目的: 普段は全員の記録を時系列で見ながら、必要なときだけグループへ絞り込み、固定の `すべて` と並べ替え可能なグループを混同しない操作を比較するため。
- 影響範囲: `frontend/public/previews/home-all-groups-options.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、未合意のグループ画面は変更しない。
- 検証: 起動中のフロントエンドからHTTP 200を確認し、Chromiumの390×844pxでA〜Dを画像確認した。グループ選択、`すべて`への復帰、右上メニュー、横スクロール、長押し並べ替えをプレビュー内だけで操作できる。
- 未解決事項: 4案の採否、個別グループを選択したときの4列集計を全体のまま残すか個別値へ切り替えるか。
- 次のアクション: ユーザーが採用案を選んだ後、固定プレビューへ反映し、全グループ横断API・重複除外・共有解除時のキャッシュを設計して実装する。

## 2026-09-26 カードとタイムライン絞り込みを分離して再比較

- 変更内容: 前回のA 数値ヘッダー、B 固定ピル、C 半幅パネルはユーザー判断で不採用とした。D案の「カードと表示範囲を分ける」考え方を共通条件にし、タイムライン直前だけをA 下線タブ、B 小型チップ、C 右上選択、D 選択バーへ変えた新しい操作可能プレビューを追加した。グループカードは絞り込みに使わず、状況確認・長押し並べ替え・詳細への入口に限定した。
- 目的: 普段は全グループ混在の時系列を見ながら、必要な場合だけ、記録一覧に近い場所で表示範囲を切り替えられる構成を比較するため。
- 影響範囲: `frontend/public/previews/home-feed-filter-options.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、グループ詳細画面は変更しない。
- 検証: Chromiumの390×844pxで4案を画像確認した。各案で朝トレ部を選択すると3件から1件へ絞られ、右上選択と下シートも開閉・選択できることをブラウザで確認した。カード列は絞り込み状態と独立して横スクロール・長押し並べ替えできる。
- 未解決事項: 新しい4案の採否。グループ数が横幅を超える場合に常時一覧性と画面の軽さのどちらを優先するか。
- 次のアクション: 採用案を固定プレビューへ反映してから、全グループ混在取得と個別絞り込みを実装する。

## 2026-09-26 下線タブを採用し、要約とグループカードを調整

- 変更内容: タイムライン絞り込みはA 下線タブを採用し、固定名の採用プレビューを追加した。上部4列は1列目だけ `LIVE` を残し、今日の人数・セット数・総負荷は説明ラベルを外して `7人 / 31セット / 28,400kg` と表示する。総負荷の `t` 表記はやめた。グループカードには名前の先頭文字による丸い仮アイコンと所属人数を追加し、カード内の総負荷もkgへ統一した。
- 目的: グループカードをタイムラインの絞り込みから切り離したまま識別しやすくし、上部の既知情報を単位だけで短く読めるようにするため。
- 影響範囲: `frontend/public/previews/home-feed-filter-options.html`、`home-feed-filter-adopted.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、グループ画像登録機能は変更しない。
- 検証: Chromiumの320×844、390×844、430×844pxで採用プレビューを画像確認し、`28,400kg`、グループアイコン、所属人数、横スクロールカード、下線タブ、START、ナビゲーションが欠けないことを確認した。
- 未解決事項: 任意画像をグループアイコンとして登録・変更する仕様。今回は既存のグループ名だけで表示できる先頭文字アイコンとする。
- 次のアクション: 採用プレビューを基準に、全グループ混在フィード、個別絞り込み、重複除外、カードの所属人数を実アプリへ実装する。

## 2026-09-26 グループカードを今日の参加者アイコンへ修正

- 変更内容: ユーザー指摘により、前回追加したグループ頭文字アイコンと総所属人数を取りやめた。代わりに、そのグループで今日トレーニング中または実施済みの人の既存アバターを重ねて表示し、今日の人数を添えた。LIVE中の人だけ赤い外周と右下点を付ける。カード下の `カードはグループ詳細への入口です` とタイムライン右側の `新しい順` は削除した。4列要約の上には短い `今日のみんなのトレーニング` を追加した。
- 目的: グループそのものの画像ではなく、今日一緒に活動している人をカードから把握できるようにし、自明な補助文を減らすため。
- 影響範囲: `frontend/public/previews/home-feed-filter-options.html`、採用プレビュー、`docs/design/home-screen.md`、`progress.md`。実アプリ、プロフィール画像、API、DBは変更しない。
- 検証: Chromiumの320×844、390×844、430×844pxで採用プレビューを画像確認した。人物アイコン、LIVEの赤い外周・点、今日の人数、短い上部説明、kg表記、下線タブが欠けず、削除対象の補助文が表示されないことを確認した。
- 未解決事項: 参加者が多い場合にカードへ表示する最大アイコン数と `+n` 表記。プレビューは先頭4人と今日の合計人数を表示する。
- 次のアクション: 人物アイコンの最大件数を実装時に既存のアバター列へ合わせ、全グループ混在フィードと下線タブを実アプリへ反映する。

## 2026-09-26 上部説明をTODAYへ変更して絞り込みを近接

- 変更内容: 4列要約上の `今日のみんなのトレーニング` を短い `TODAY` に変更した。`みんなのトレーニング` の見出し行を11px縮め、下線タブの高さも4px縮めて、見出しと絞り込みの間隔を詰めた。
- 目的: 上部の説明をデザインに馴染む短い言葉にし、タイムライン見出しと表示範囲の選択が一つの操作領域として読めるようにするため。
- 影響範囲: `frontend/public/previews/home-feed-filter-options.html`、採用プレビュー、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DBは変更しない。
- 検証: Chromiumの320×844、390×844、430×844pxで採用プレビューを確認し、TODAY、見出し、下線タブが欠けず、タブによる絞り込みを維持することを確認する。
- 未解決事項: なし。
- 次のアクション: 採用プレビューを基準に実アプリへ反映する。

## 2026-09-26 多種目・多セット時のホーム表示を確認

- 変更内容: 採用済みホームを保ったまま、1件目を5種目・22セット、2件目を2種目・8セットとした負荷確認用プレビューを追加した。トレーニングを種目ごとのカードへ分割せず、1枚の記録カード内へ全種目・全セットを縦に表示する。
- 目的: 種目数・セット数が多い実利用時のカードの伸び方、スクロール量、浮遊STARTと下部ナビゲーションとの重なり方を実画面で判断できるようにするため。
- 影響範囲: `frontend/public/previews/home-feed-filter-options.html`、`frontend/public/previews/home-feed-many-sets.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、保存済みデータは変更しない。
- 検証: Chromiumで320×844、390×844、430×844pxを確認した。各幅で本文の横はみ出しがなく、先頭カードの5種目・22セットと最後のセット行が同じカード内に収まることを確認した。先頭カードの高さは950pxで、ページの縦スクロールにより全内容を閲覧できる。
- 未解決事項: 長い記録を初期状態から全展開するか、概要の後に折りたたむかはプレビュー確認後に判断する。
- 次のアクション: スマホで多種目・多セット版を確認し、全展開を維持するかを決める。

## 2026-09-26 長い記録カードの4案を比較

- 変更内容: 同じ5種目・22セットの記録で、A 全展開、B 固定高＋下端フェード、C 種目単位の開閉、D 概要カード＋下から開く詳細の4案を切り替えられる静的プレビューを追加した。Bはその場で全展開・再折りたたみ、Cは先頭種目だけを初期展開して各種目を個別開閉、Dは詳細をモーダル表示できる。
- 目的: 長いトレーニングがタイムラインを占有する問題を、共通の記録カード構造と全セットへの到達性を保ちながら比較するため。
- 影響範囲: `frontend/public/previews/home-feed-filter-options.html`、`frontend/public/previews/home-long-record-options.html`、`docs/design/home-screen.md`、`progress.md`。実アプリ、API、DB、保存済みデータは変更しない。
- 検証: Chromiumの390×844pxで4案を表示し、横はみ出しがないことを確認した。先頭カードはA 954px、B 590px、C 547px、D 468px。Bの全展開後は1004px、Cは2種目を同時に開け、Dの詳細には全5表が表示された。
- 未解決事項: 4案の採否。決定まではAの全展開を現在の実装基準として維持する。
- 次のアクション: スマホで4案を比較し、採用案を固定プレビューと実アプリへ反映する。

## 2026-09-26 長い記録カードに固定高＋フェードを採用

- 変更内容: 4案からB 固定高＋下端フェードを採用した。ホームの小型記録カードは種目・セット部分を最大352pxに抑え、内容が続く場合だけ文字のないフェード操作を表示する。フェードを押すと同じカード内へ全セットを一度だけ展開する。比較・多セット・ホーム絞り込みの採用プレビューもBへ更新した。
- 目的: 多種目・多セットの記録がタイムラインを占有し続ける問題を抑えつつ、別画面へ移動せず全セットを読めるようにするため。可視の `全て表示` はフェードと役割が重複するため置かない。
- 影響範囲: ホームの`compact`記録カード、長い記録カードの比較・採用プレビュー、関連E2E、`docs/design/home-screen.md`、`progress.md`。履歴・スタンプ詳細・トレーニング中の通常記録カード、API、DB、保存済みデータは変更しない。
- 検証: 先に5種目・22セットのE2Eへ、初期状態の固定高・フェード操作・押下後の全展開を追加し、未実装時に失敗することを確認した。実装後は関連E2E 2件、frontend lint、型検査、単体テスト99件、production buildが成功した。比較プレビューは320・390・430pxで横はみ出しがなく、採用Bは長いカードだけにフェード操作が出ることを確認した。
- 未解決事項: なし。
- 次のアクション: 実機スマホでフェードの濃さと押しやすさを確認する。

## 2026-09-26 記録カードの往復開閉とSTART幅固定

- 変更内容: ホームの長い記録は、フェードだけでなくカード内の非操作領域を押しても全展開し、再度押すと固定高へ戻るようにした。スタンプなどのボタン操作は開閉から除外した。浮遊する`START / RESUME`は112×52pxへ固定し、長押し移動中の拡大を廃止して影と軽い透明度だけで状態を示す。
- 目的: フェードの狭い領域を狙わずカード全体から開閉できるようにし、移動時に開始ボタンが横へ膨らむ視覚的な揺れをなくすため。
- 影響範囲: ホームの`compact`記録カード、浮遊する開始・再開操作、採用プレビュー、関連E2E、`docs/design/home-screen.md`、`docs/design-system.md`、`progress.md`。スタンプ送信、記録保存、通常サイズの記録カード、API、DBは変更しない。
- 検証: 記録の種目名から展開・再縮小できること、フェードからも展開できること、START長押し中と移動後も幅が変わらないことを先にE2Eへ追加し、未実装時に両方失敗することを確認した。実装後は関連E2E 8件、frontend lint、型検査、単体テスト99件、production buildが成功した。
- 未解決事項: なし。
- 次のアクション: 実機スマホでカードのタップとスタンプ操作が競合しないこと、STARTの移動感を確認する。

## 2026-09-26 長い記録カードのフェードを強調

- 変更内容・目的: ユーザーの実機確認を受け、ホームの長い記録カードのフェード操作領域を86pxから104pxへ広げた。白く消す表現から、下端へ向かって最大16%の薄いチャコールを重ねる表現へ変更し、続きとタップ領域を文字なしでも把握しやすくした。
...
## 2026-09-26 02:15 ホームのカードとタイムライン切替を分離

- 変更内容: ホームの上段グループカードが選択グループ別APIとタイムラインを切り替える旧実装を外し、今日の全グループ取得結果から重複なし・最新順の `すべて` を組み立てるようにした。絞り込みは `みんなのトレーニング` 直下の `すべて / 各グループ` だけで即時に行う。上段カードはスワイプ表示、長押し並べ替え、詳細入口だけを担当する。TODAYの4列表示、今日の仲間アバター、人数・セット・kgを採用プレビューどおりカードへ追加し、ホーム下部の `届いたスタンプ` と `前回を振り返る` を削除した。横一列の部位・グループ・指標・スタンプ絞り込みは、既存の分析タブと同じ薄いトラック＋白い選択面へ統一した。
- 目的: グループカードを動かした瞬間に旧デザインや前の記録が見える切替をなくし、普段は全員の頑張りを一つの時系列で見ながら必要な場合だけ下段で絞り込めるようにするため。同じ目的の選択操作も同じ見た目へ揃える。
- 影響範囲: `frontend/src/features/v2/community.tsx`、`home-feed.ts`、`use-shared-workout-details.ts`、`workspace.tsx`、`frontend/src/app/v2.css`、関連単体/E2E、`docs/design/home-screen.md`、`docs/design-system.md`、`progress.md`。API・DB・共有権限・グループ詳細の情報構成は変更しない。
- 検証: 全グループ表示の重複除外、最新順、個別絞り込み、記録ごとの共有先解決を単体テストへ追加した。frontend lint、型検査、単体テスト101件、production buildが成功した。ホームの上下操作分離、カード表示、補助欄削除、共通選択表示、STARTの320/390/430pxを含む対象E2E 14件が成功した。受信スタンプE2E 3件は今回変更したホーム到達前に、現行の記録画面に `記録中のスタンプ` が存在しないため失敗した。今回のホーム変更に起因しない既存実装とテストの差分として成功扱いにせず残す。
- 未解決事項: グループ数または今日の記録が非常に多い場合のページング表現は従来どおり保留。
- 次のアクション: 実機スマホで全員表示、下段絞り込み、横スワイプ、カード詳細を確認する。記録中の受信スタンプを常設へ戻すかは、現行の入力配置と合わせて別途確認する。

## 2026-09-26 02:40 ホームのLIVE枠・選択位置・空状態を調整

- 変更内容: グループカードのLIVEアバターで共通部品の赤い外周とカード固有の赤い境界が重なっていたため、カード固有の赤線を外して外周1本と右下点だけにした。タイムラインのグループ選択は、項目が収まる場合に全体を中央寄せし、あふれる場合は安全に先頭から横スクロールする。空状態を `みんなのトレーニングを待っています` へ変更した。
- 目的: LIVE表示の二重線をなくし、少数グループ時の選択列を安定して中央へ見せ、ホームに合う前向きな空状態へ揃えるため。
- 影響範囲: ホームのグループカード、タイムライン表示範囲、空状態、関連E2E、`docs/design/home-screen.md`。API・DB・集計・共有範囲は変更しない。
- 検証: frontend lint、型検査、単体テスト101件が成功した。LIVE外周とカード境界の色、選択列の左右余白、空状態文言を確認するE2Eを追加し、`community-v2.spec.ts` 7件が成功した。スマホ用フロントエンドとAPIのHTTP 200も確認した。
- 未解決事項: なし。
- 次のアクション: 実機スマホでLIVEアバター、選択列、記録がないグループの空状態を確認する。

## 2026-09-26 02:52 種目選択のヘッダーと選択列を再調整

- 変更内容: トレーニング中のヘッダーをホームと同じ48px・ロゴ22pxへ固定した。仲間のグループ選択はホームのタイムライン絞り込みと同じ35px高・11px文字・9px角丸・中央寄せへ揃えた。部位選択はユーザー確認を受けて2段案を取り下げ、8部位を横スクロールなしの一列8等分とし、同じ高さ・文字・角丸と4pxの外周余白を使う形へ整えた。採用プレビューと設計資料も更新した。
- 目的: 共通の選択色だけを無理に適用して寸法が不揃いだった状態を直し、ホームから種目選択へ移動した際のヘッダーのガタつきと、端の部位を押した際の窮屈さをなくすため。
- 影響範囲: トレーニング中の種目選択・記録入力ヘッダー、仲間のグループ選択、部位選択、採用プレビュー、関連E2E、`docs/design/training-session.md`、`docs/design-system.md`。候補絞り込み、記録保存、API、DBは変更しない。
- 検証: 先にホームと異なるグループボタン寸法でE2Eが失敗することを確認した。実装後はホームとトレーニングのヘッダー高、グループと部位ボタンの高さ・文字・角丸が一致し、320・390・430pxすべてで8部位が一列、等幅、横スクロールなしになる対象E2Eが成功した。frontend lint、型検査、単体テスト101件、production buildも成功した。
- 未解決事項: なし。
- 次のアクション: 実機スマホでホームから種目選択・記録入力を往復し、ヘッダーと端の部位の押下感を確認する。

## 2026-09-26 03:00 feed要約のbackend期待値を総負荷へ追従

- 変更内容: PR #196のbackend CIで、ホーム用feed要約へ追加済みの`total_volume`と既存テストの完全一致期待値がずれていることを確認した。保存後の2400kgと、自重記録だけへ編集した後の0kgを期待値へ加え、共有記録要約の仕様にも総負荷の算出方法を追記した。
- 目的: 意図したAPI追加を維持したまま、種目数・セット数・総負荷が保存後と履歴編集後に一緒に更新されることを回帰テストで保証するため。
- 影響範囲: backendのfeed要約テスト、`docs/shared-workout-summary.md`、変更履歴。API実装、DB、画面は変更しない。
- 検証: PR #196のbackend CIで旧期待値が失敗することを確認済み。backend lintが成功し、DB未指定のローカルテストは103件成功・DB依存171件skipだった。専用PostgreSQLを使うPR CIで対象テストを含む全274件を再確認する。
- 未解決事項: なし。
- 次のアクション: backend検証後にPR #196へpushし、全CI成功まで監視する。

## 2026-09-26 03:06 320pxのトレーニングヘッダー横はみ出しを修正

- 変更内容: PR #196のdatabaseジョブ内E2Eで、320pxの記録画面だけ文書幅が325pxになることを確認した。狭幅時に本文の左右余白が10pxへ変わるのに、ヘッダーの負マージンが16pxのままだったため、350px以下では負マージンも10pxへ合わせた。E2Eへヘッダーの左端0px・幅がviewportと一致する確認を追加した。
- 目的: ホームと同じ48pxヘッダーを維持しつつ、320pxで左右へ5pxずつはみ出して横揺れする状態をなくすため。
- 影響範囲: トレーニング中の共通ヘッダー、記録画面の表示E2E、`docs/design/training-session.md`。入力、保存、API、DBは変更しない。
- 検証: 修正前にローカルでも320pxだけ`scrollWidth=325`となり、画面外要素が`.session-header`だけであることを確認した。修正後は320・390・430pxの対象E2E 3件が成功し、ヘッダーがviewport内に収まり文書幅とviewport幅が一致することを確認した。
- 未解決事項: なし。
- 次のアクション: 修正をpushし、PR #196の全CI成功を確認する。
