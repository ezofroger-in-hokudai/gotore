...
## 2026-09-14 00:20
- 変更内容: #143の対象をVercelのヒートマップへ特定。ユーザー提供のテストアカウントでヒートマップ・種目一覧503、履歴一覧・分析200を再現。公開Supabaseを0件の列取得で確認し、primary_body_part・secondary_body_parts・revision不在（42703）を確認した。
- 目的: 再試行では解消しないDB更新不足を特定し、接続障害との混同をなくす。接続/SQL/返却の分類ログを追加し、秘密情報・記録本文・実URLは記録しない。
- 検証: ローカル既存データ195ユーザー月の集計成功。先行診断テスト7件の失敗後に実装し成功。部位列不足を専用_test DB内で再現し、既存2migrationの追加適用で種目一覧・月集計が復旧、記録本文が不変であるテストを追加。対象14件成功。make check成功（backend245件・frontend75件、lint・型・build）。公開画面でテストアカウントの閲覧を確認し、ローカルの記録/共有E2Eは後続の画面変更とまとめて実行する。
- 影響範囲・関連ファイル: dependencies.py、test_database_connection.py、test_body_part_activity.py、docs/vercel-supabase.md、task.md。API応答・保存処理・既存migrationは変更しない。
- 未解決事項: 公開DBへの管理アクセスは403。公開DBは変更していない。ユーザーが2migrationの適用を担当すると回答。公開環境の適用後確認は未実施。
- 次のアクション: 標準チェックを実行して診断PRへ記録。別コミット/PRで合意済み文言整理と筋トレキャラクターの読み込み表示へ進む。
