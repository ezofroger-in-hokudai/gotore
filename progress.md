...
## 2026-09-13 15:25
- 変更内容: ヒートマップから部位を見る画面の3案比較に着手。前回PR #135はマージ済みのため、main aa793f5からdocs/heatmap-body-part-proposalsを作成。
- 目的: 情報密度と操作の違いを実際の画面で確認してから採用する。
- 影響範囲: docs配下の静的な架空データの試作。アプリ・API・DBは変更しない。
- 関連ファイル: docs/activity-heatmap.md、docs/exercise-body-parts.md、task.md。
- 検証方針: 見た目を比較する可逆な文書試作なので実アプリの先行テストは追加しない。ブラウザで操作・狭い幅・空日・0kg・色・補助部位表示を検証する。
- 未解決事項: 採用案は未決定。主部位別に集計し補助部位を種目のタグで見せる仮案を使い、負荷を二重計上しない。これは部位別の生理的な負荷配分ではない。
- 次のアクション: A/B/Cの操作画面と比較画像を用意し、元のリポジトリのdocs/previewsにも置いて選択を依頼する。

## 2026-09-13 15:35
- 変更内容: #136のA（日付から部位詳細）/B（部位別の月表示）/C（部位×曜日）の操作できるHTMLと比較画像・個別画像を作成。主部位で集計し補助は種目のタグとする仮案、月/週内の色尺度と未分類の扱いを比較資料に明記。
- 目的: 実アプリへ接続する前に情報量と操作を選べるようにする。推奨は通常カレンダーの項目を増やさないA。
- 影響範囲: docs・task/progressのみ。元リポジトリのdocs/previews/heatmap-body-parts.htmlとdocs/images/heatmap-body-partsにも同じファイルを配置し、別の作業フォルダを探さず確認できるようにした。
- 関連ファイル: docs/heatmap-body-part-proposals.md、docs/previews/heatmap-body-parts.html、docs/images/heatmap-body-parts/、docs/README.md。
- 検証: ローカルChromiumで3案×320/390/430pxの横はみ出しなし。日付詳細、Escape・フォーカス復帰、部位/比較案切替、月/週境界、未来日の無効、空部位、未記録、0kg、未分類、主補の二重計上なしを確認。JavaScriptエラー0・外部HTTP通信0。C案の数値/色の説明をマスの上へ移し、最終画像を再撮影。関連資料と画像のリンク確認、git diff --check成功。静的な画面見本のためmake check・実DB E2Eは実施しない。
- 未解決事項: 採用案と、実データの分類変更時の扱いは未決定。試作の表示は実データの性能・集計・権限を保証しない。
- 次のアクション: 画像付きの画面比較PRを提出し、ユーザーにA/B/Cを選んでもらう。採用後に実装条件を整理する。

## 2026-09-13 15:38
- 変更内容: ヒートマップと部位の3案を画像付きドラフトPR #137へ提出。https://github.com/ezofroger-in-hokudai/gotore/pull/137
- 目的: 操作と情報量を比べ、採用する画面を決める。
- 影響範囲: 画面試作と記録のみ。主な検証は直前の試作完成記録を参照。
