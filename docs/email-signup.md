...
4. URL ConfigurationのSite URLを実際の公開URLへ設定する。現行の案内先は `https://gotore-two.vercel.app/`。Redirect URLsに `https://gotore-two.vercel.app/auth/confirm` を正確に追加する。別ドメインなら両方をそのドメインへ置き換える。ProductionにPreview用ワイルドカードを追加せず、Previewを試す場合は専用Authと正確なPreview URLを使用する。
