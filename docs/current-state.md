...
2026-09-15に採用したプレビューを基準に、カレンダー／グラフと選択期間の記録を共通化する。期間幅に合う粒度、7日分の実施マーク、グループの活動日数・人数と単一メンバーの重量推移を提供する。月・日付の指定とスワイプで過去へ移動できる。共有範囲・50件単位の読み込み・本人の編集/コピー・ランキングを維持し、追加migrationは不要。詳細は [履歴グラフ・グループ集計](history-analytics.md#2026-09-15-採用した統一表示)。

## 設定の目安箱（#189）

追加実装ブランチでは設定のサポートから目安箱へ送信する。ログインした本人の投稿を非公開テーブルへ保存し、運営はSupabaseのTable Editorで確認する。一般ユーザー用の読取APIは設けない。migrationを先に適用する。[仕様・管理手順](suggestion-box.md)。
