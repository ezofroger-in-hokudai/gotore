...
## グループ切替と起動の追加改善（2026-09-15）

PR #155後のユーザー報告を受け、スワイプ中の選択変更を止め、指を離した時に隣の1グループへ確定する。隣接フィードを最大2要求・5件のメモリ保持で準備する。認証後もホームに必要な初回取得までウォンバットを表示し、初回ガイドは準備後に開く。詳細は[読み込み仕様](loading-performance.md#2026-09-15-起動時の表示準備と隣接グループの先読み)と[画面例](images/group-startup/README.md)。API・DB・環境変数の追加はない。実機の操作感は#23で確認する。
