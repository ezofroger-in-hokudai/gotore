...
#118の追加指定で、ホームのカード・グループ一覧の長押しから表示順を変更する。順序はこのブラウザの本人専用設定に保存し、左右のカードと一覧で共用する。現在の所属だけを表示し、共有範囲と選択中グループを変更しない。API・DB・migrationの追加はない。詳細は[グループ表示順](gotore-v2-spec.md#グループ表示順118)。
