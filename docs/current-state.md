...
| 機能 | 動作 | 主な配置先 |
| --- | --- | --- |
| アカウント | メール確認付き一般登録・再送、ログイン・ログアウト。管理者発行も継続。設定で本人の表示名を取得・変更・同期。Auth未設定時はDBの既存名を保持（#29）。APIがトークンと認証設定の形式を検証 | frontend/src/features/settings/、auth-panel.tsx、backend/app/api/dependencies.py |
...
Vercel Servicesでは1プロジェクトの共通ルートから各サービスへ振り分けます。設定は [vercel-supabase.md](vercel-supabase.md) を参照してください。ユーザーが公開サイトをデプロイし、migration適用を報告済みです。公開Authの一般登録・SMTP・確認URL設定は [メール登録の公開手順](email-signup.md#公開への反映手順)に従って管理者が反映します。コードだけでは公開設定は更新されません。各リリースの本番反映は管理者が確認します。
...
- E2E: メール確認付き登録と未確認ログイン拒否、管理者作成アカウントでのログイン・グループ作成・参加・記録共有、通信再試行、下書き復元、再ログイン。
...
#102/#103では開始ボタンの後に種目選択し、開始前には履歴と共用する直近記録・総負荷の推移を表示する。新規SCOREの比較不能項目は承認済みの初回基準で補う。詳細はloading-performance.mdとscore-implementation.mdを参照。一言の共有表示は後続 #104。


## グループ表示順

#118の追加指定で、ホームのカードを長押ししてそのまま左右に移動し、離すと表示順を保存する。グループ一覧の長押し/並べ替えシートでは上下ボタンでも操作できる。順序はこのブラウザの本人専用設定に保存し、左右のカードと一覧で共用する。現在の所属だけを表示し、共有範囲と選択中グループを変更しない。API・DB・migrationの追加はない。詳細は[グループ表示順](gotore-v2-spec.md#グループ表示順118)。

#134の追加依頼では、最高記録を重量/RM別のフラグとして本人履歴・共有詳細へ一括して付け、記録中は既存の比較応答を使う。炎と赤字の統一表示は[最高記録の表示](personal-record-highlights.md)を参照。新規migrationは不要。既存の集計テーブルから最高値を読み、一覧ごとのSQLにまとめる。

## 日別の部位表示と横一列フィルター（#138）

ユーザー指定により、日付セルに部位を表示し、固定8分類を小さい文字の横一列ボタンで絞り込む。全身・未設定はその他へ統一するmigrationを追加。現在の主部位を過去記録にも適用し、月内全体/部位別を既存統計からSQL1回で取得する。表示・集計仕様は[部位ヒートマップ](body-part-calendar.md)、重複見出し整理は[画面文言](ui-copy.md)を参照。


## 部位の複数選択と文言整理（#140）

履歴カレンダーの部位を複数選択し、選んだ現在の主部位の総負荷とセット数を合算する。月APIの部位組合せ別件数からトレーニング件数の重複を除き、切替は月キャッシュ内で完結する。凡例と常設説明を使い方へ移し、主要画面の明確な重複文言を整理する。仕様は[部位ヒートマップ](body-part-calendar.md)と[画面文言](ui-copy.md)。PR #139が前提で、新たなmigrationはない。
