...
追加指定により、胸・背中・脚・腕・肩・腹筋・お尻・その他を固定表示。上部は「すべて」を含む9ボタン、11px文字・44px高さで横一列に収める。固定5項目＋その他の開閉案は採用しない。

[操作するHTML](previews/heatmap-body-parts-fixed.html) / [390px画像](images/heatmap-body-parts/fixed-390.png)

320/390/430pxで文字切れ・横スクロール・折り返しがないことを確認。試作の架空データでは旧未分類をその他にまとめた。実データの全身・未設定をその他へ統合するかは確認中であり、移行はしていない。実装PR #139はこの追加指定を反映するためドラフトを維持する。

以下は比較の経緯。横スクロール案は最新案で置き換える。


2026-09-13のユーザー依頼による試作。初期3案の比較後、横一列フィルターと日付内の部位表示、現在分類を過去にも適用する方式を採用した。実アプリへの接続は #138 で進める。元となる現行仕様は[総負荷ヒートマップ](activity-heatmap.md)と[種目の部位分類](exercise-body-parts.md)。
...
[修正版を操作](previews/heatmap-body-parts-refined.html) / [390pxの画像](images/heatmap-body-parts/refined-390.png)

- 日付のマスに部位名を表示。複数は「胸+1」と省略し、タップで全内訳を確認する。
- 部位フィルターは全項目を横一列にし、折り返さず横スクロール。
- 「総負荷カレンダー」の表示見出しを削除。
- ユーザー回答で「現在の分類で過去も表示し直す」を採用した。

以下のA/B/Cは初期の比較履歴。推奨案の記述も当時の判断として残す。
