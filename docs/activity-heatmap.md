...
日付に主部位を表示し、横一列の部位フィルターで月データを絞る。現在の種目分類を過去にも適用する。集計・表示・互換性の追加仕様は[部位ヒートマップ](body-part-calendar.md)を参照。

## 2026-09-13の総負荷量への変更（#130・現行仕様）

ユーザー依頼によりスコア機能を一時停止する。以下のSCORE版・セット数版は経緯として残す。

- 日の指標は本人の同日の全記録・全種目の `重量(kg) × 回数` の合計。同日複数回も合算し、共有先の数では重複させない。未終了の保存済みセット、過去の未採点記録も含める。
- 0kgの記録日は0kg（青）として、未記録日（灰色）と区別する。自重の重量を推測で補完しない。
- 月の要約は総負荷kg・活動日数・記録件数。各日にはkg（1k＝1,000kg）を表示し、読み上げには省略しない値を使う。
- 色は月内最大の総負荷量を赤、0kgを青とする連続グラデーション。緑・黄・オレンジを通り紫を使わない。月内の相対表示であることを明示する。色基準の任意確認には未回答のため、この基準を暫定採用する。
- `GET /api/workouts/activity?month=YYYY-MM` は `metric: "volume"`、月の `total_volume`、各 `days[].volume` を返す。既存の `total_sets`、件数、日数も保持する。全期間JSONを読み出さず、既存 `gotore_workout_statistics.volume` をDBで合計する。
- 本人限定・日本時間の記録日・2000年以降・月境界・ページ送り・未来日無効・エラー表示を維持する。保存・編集・削除は統計の既存トリガーで反映し、既存の先読み・メモリキャッシュ・再確認を利用する。
- ホーム・履歴・共有詳細のスコア表示、終了後の採点/AIコメント、設定の目標とグループ配点を停止する。終了画面は保存済み記録から総負荷・種目数・セット数を即時表示する。
- スコア/目標APIは公開ルーターから外して404を返し、開始・終了・編集・一覧で採点を実行しない。旧レスポンスの `score` は互換用にnullを返す。既存DBの得点・目標を削除しない。
- 新規migration `20260913010000_pause_score_observation.sql` は採点専用の記録観測トリガーだけを停止する。通常の記録統計・共有・RLSは維持する。過去のmigrationは変更しない。
...
色はホーム・終了画面・履歴のバッジと共通で、未評価/未記録は灰色。0点の青から緑・黄・オレンジを通り、100点のテーマカラーの赤へ連続的に変化する。紫を使わず、高得点ほど赤に近づく。月間最大値に合わせて変えない。色だけでなく点数・日付を表示する。月の要約は最高SCORE・活動日数・記録件数。
