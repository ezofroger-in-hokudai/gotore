...
- [メール確認付き登録の公開手順](email-signup.md#公開への反映手順)に従い、SMTP・確認URL・テンプレートを準備後に一般登録を有効にする。Confirm Emailは有効のまま維持する。公開プロジェクトの設定は管理者が行う。
...
- 管理者が発行した2ユーザーでログイン・グループ作成／参加・記録共有・再ログインができる。共有外のユーザーには見えない。一般登録→確認メール→利用開始と、確認前のログイン拒否・再送・期限切れを確認する。
