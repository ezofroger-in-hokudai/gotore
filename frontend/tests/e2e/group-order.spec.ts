...
for (const width of [320, 390, 430]) {
  test(`${width}px: 長押しで並べ替え、選択を保ちホームと一覧・再読込に反映する`, async ({
    ...
    await hold(page, page.getByRole("button", { name: "画面テスト部の詳細", exact: true }));
    const sheet = page.getByRole("dialog");
    await sheet.getByRole("button", { name: "画面テスト部を下へ", exact: true }).click();
    await expect(sheet.locator(".group-order-name")).toHaveText([names[1], names[0], names[2]]);
...
test("タッチの横スクロールは長押しを起動せず、長押し後にハンドルを動かせる", async ({ page }) => {
  await setup(page);
  ...
  const a = await sheet
    .getByRole("button", { name: "画面テスト部を移動", exact: true })
    .boundingBox();
  const b = await sheet.getByRole("button", { name: "朝トレ部を移動", exact: true }).boundingBox();
  if (!a || !b) throw new Error("移動先がありません");
  await cdp.send("Input.dispatchTouchEvent", {
    type: "touchStart",
    touchPoints: [{ x: a.x + 24, y: a.y + 24 }],
  });
  await cdp.send("Input.dispatchTouchEvent", {
    type: "touchMove",
    touchPoints: [{ x: b.x + 24, y: b.y + 24 }],
  });
  await cdp.send("Input.dispatchTouchEvent", { type: "touchEnd", touchPoints: [] });
  await expect(sheet.locator(".group-order-name")).toHaveText([names[1], names[0], names[2]]);
