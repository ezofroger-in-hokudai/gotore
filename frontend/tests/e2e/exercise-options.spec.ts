...
test("本人の種目リストを追加・選択・削除し、下書きと失敗時の入力を保持する", async ({ page }) => {
  ...
  await page.getByText("自分の種目リストを管理", { exact: true }).click();
  const name = page.getByLabel("追加する種目名", { exact: true });
  await name.fill(" ケーブルロウ ");
  state.failOptionWrite = true;
  await page.getByRole("button", { name: "リストに追加", exact: true }).click();
  ...
  await expect(exercise).toHaveValue("ケーブルロウ");
  await expect(exercise.locator("option:checked")).toHaveText("ケーブルロウ（下書きの種目）");
  ...
  await page.getByRole("button", { name: "記録を確定して共有 →", exact: true }).click();
...
test("候補取得の失敗は再試行でき、空リストからも種目を追加できる", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "＋ トレーニングを記録", exact: true }).click();
  await expect(page.getByRole("alert").filter({ hasText: "通信できません" })).toBeVisible();
  await expect(page.getByText("種目リストは空です。", { exact: false })).toHaveCount(0);
  state.failOptions = false;
  state.options = [];
  await page.getByRole("button", { name: "種目リストを再取得", exact: true }).click();
  await expect(page.getByText("種目リストは空です。", { exact: false })).toBeVisible();
  await page.getByText("自分の種目リストを管理", { exact: true }).click();
  await page.getByLabel("追加する種目名", { exact: true }).fill("新しい種目");
  await page.getByRole("button", { name: "リストに追加", exact: true }).click();
  const exercise = page.getByRole("combobox", { name: "種目名", exact: true });
